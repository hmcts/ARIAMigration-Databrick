...
def test_allFtpaAppellantDecisionDocs(documents_outputs):
    r = documents_outputs

    assert r["CASE005"]["allFtpaAppellantDecisionDocs"] == []
    assert r["CASE006"]["allFtpaAppellantDecisionDocs"] == []
    assert r["CASE007"]["allFtpaAppellantDecisionDocs"] is None
...
def test_ftpaAppellantNoticeDocument(documents_outputs):
    r = documents_outputs

    assert r["CASE005"]["ftpaAppellantNoticeDocument"] == []
    assert r["CASE006"]["ftpaAppellantNoticeDocument"] == []
    assert r["CASE007"]["ftpaAppellantNoticeDocument"] is None
    assert r["CASE010"]["ftpaAppellantNoticeDocument"] == []
    assert r["CASE011"]["ftpaAppellantNoticeDocument"] is None
