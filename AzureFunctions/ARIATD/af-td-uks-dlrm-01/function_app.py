...
@app.event_hub_message_trigger(
    arg_name="tdazeventhub",
    event_hub_name="evh-td-pub-dev-uks-dlrm-01",
...
async def eventhub_trigger(tdazeventhub: List[func.EventHubEvent]):
    logging.info(f"Processing a batch of {len(tdazeventhub)} events")
    ...
    try:
        ...
        ev_dl_key = kv_client.get_secret("evh-td-dl-uks-dlrm-01").value
        ev_ack_key = kv_client.get_secret("evh-td-ack-uks-dlrm-01").value
        ...
        container_secret = kv_client.get_secret("ARIATD-SAS-TOKEN").value
        ...
        async with ContainerClient.from_container_url(container_url) as container_service_client, \
            ...
            tasks = [
                process_messages(event, container_service_client, "ARIATD/submission", dl_producer_client, ack_producer_client)
                for event in tdazeventhub
            ]
