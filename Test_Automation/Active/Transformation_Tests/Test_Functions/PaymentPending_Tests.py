...
def test_internalAppellantMobileNumber_ac1(json, M2_bronze):
    try:
        ...
        error_message = str(e)        
        return TestResult("internalAppellantMobileNumber", "FAIL",f"TEST FAILED WITH EXCEPTION :  Error : {error_message[:300]}", test_from_state, inspect.stack()[0].function)
...
def test_mobileNumber_ac1(json, M2_bronze):
    try:
        ...
        error_message = str(e)        
        return TestResult("mobileNumber", "FAIL",f"TEST FAILED WITH EXCEPTION :  Error : {error_message[:300]}", test_from_state, inspect.stack()[0].function)
...
def test_oocAppealAdminJ_ac1(test_df):
    try:
        ...
        if ac_oocAppealAdminJ.count() != 0:
            return TestResult("oocAppealAdminJ", "FAIL", f"oocAppealAdminJ acceptance criteria failed: {str(ac_oocAppealAdminJ.count())} cases have been found where CategoryId is not 38 and oocAppealAdminJ is not omitted" , test_from_state, inspect.stack()[0].function)
        else:
            return TestResult("oocAppealAdminJ", "PASS", f"oocAppealAdminJ acceptance criteria passed, where CategoryId is not 38, oocAppealAdminJ is always omitted.", test_from_state, inspect.stack()[0].function)
    except Exception as e:
        error_message = str(e)        
...
def test_oocAppealAdminJ_ac2(test_df):
    try:
        ...
        if ac_oocAppealAdminJ.count() != 0:
            return TestResult("oocAppealAdminJ", "FAIL", f"oocAppealAdminJ acceptance criteria failed: {str(ac_oocAppealAdminJ.count())} cases have been found where CategoryId is 38 + CleansedHORef contains GWF and oocAppealAdminJ is not 'entryClearanceDecision'." , test_from_state, inspect.stack()[0].function)
        else:
            return TestResult("oocAppealAdminJ", "PASS", f"oocAppealAdminJ acceptance criteria passed, where CategoryId is 38 + CleansedHORef contains GWF, oocAppealAdminJ is always 'entryClearanceDecision'.", test_from_state, inspect.stack()[0].function)
    except Exception as e:
        error_message = str(e)        
        return TestResult("oocAppealAdminJ", "FAIL",f"TEST FAILED WITH EXCEPTION :  Error : {error_message[:300]}", test_from_state, inspect.stack()[0].function)
...
def test_oocAppealAdminJ_ac3(test_df):
    try:
        ...
        if ac_oocAppealAdminJ.count() != 0:
            return TestResult("oocAppealAdminJ", "FAIL", f"oocAppealAdminJ acceptance criteria failed: {str(ac_oocAppealAdminJ.count())} cases have been found where CategoryId is 38 + M1.HORef contains GWF and oocAppealAdminJ is not 'entryClearanceDecision'" , test_from_state, inspect.stack()[0].function)
        else:
            return TestResult("oocAppealAdminJ", "PASS", f"oocAppealAdminJ acceptance criteria passed, where CategoryId is 38 + M1.HORef contains GWF, oocAppealAdminJ is always 'entryClearanceDecision'.", test_from_state, inspect.stack()[0].function)
    except Exception as e:
        error_message = str(e)        
        return TestResult("oocAppealAdminJ", "FAIL",f"TEST FAILED WITH EXCEPTION :  Error : {error_message[:300]}", test_from_state, inspect.stack()[0].function)
...
def test_sponsorPartyId_ac3(test_df):
    try:
        ...
        if ac_sponsorPartyId.count() != 0:
            return TestResult("sponsorPartyId", "FAIL", f"sponsorPartyId acceptance criteria failed: {str(ac_sponsorPartyId.count())} cases have been found where CategoryId is not 38 and 'sponsorPartyId' is not omitted", test_from_state, inspect.stack()[0].function)
        else:
            return TestResult("sponsorPartyId", "PASS", f"sponsorPartyId acceptance criteria passed, where 'CategoryId' is not 38 all 'sponsorPartyId' are omitted", test_from_state, inspect.stack()[0].function)
    except Exception as e:
        error_message = str(e)        
        return TestResult("sponsorPartyId", "FAIL",f"TEST FAILED WITH EXCEPTION :  Error : {error_message[:300]}", test_from_state, inspect.stack()[0].function)
...
def test_recordedOutOfTimeDecision_ac1(json, M3_bronze, M1_bronze):
    try:
        ...
        if test_df != None:
            ...
            if ac.count() != 0:
                return TestResult("recordedOutOfTimeDecision", "FAIL", f"recordedOutOfTimeDecision acceptance criteria failed: {str(ac.count())} cases have been found where M1.OutOfTimeIssue is 1 + M3.Outcome != 0 and recordedOutOfTimeDecision != Yes", test_from_state, inspect.stack()[0].function)
            else:
                return TestResult("recordedOutOfTimeDecision", "PASS", f"recordedOutOfTimeDecision acceptance criteria passed, all cases where M1.OutOfTimeIssue is 1 + M3.Outcome != 0 have recordedOutOfTimeDecision = Yes", test_from_state, inspect.stack()[0].function)
        else:
...
def test_recordedOutOfTimeDecision_ac2(json, M1_bronze, M3_bronze):
    try:
        ...
        if test_df != None:
            ...
            if ac.count() != 0:
                return TestResult("recordedOutOfTimeDecision", "FAIL", f"recordedOutOfTimeDecision acceptance criteria failed: {str(ac.count())} cases have been found where M1.OutOfTimeIssue is 1 + M3.Outcome == 0 and recordedOutOfTimeDecision is not omitted", test_from_state, inspect.stack()[0].function)
            else:
                return TestResult("recordedOutOfTimeDecision", "PASS", f"recordedOutOfTimeDecision acceptance criteria passed, all cases where M1.OutOfTimeIssue is 1 + M3.Outcome = 0 have recordedOutOfTimeDecision is omitted", test_from_state, inspect.stack()[0].function)
        else:
            return TestResult("recordedOutOfTimeDecision", "FAIL",f"Failed to Setup Data for Test - recordedOutOfTimeDecision does not exist in the payload", test_from_state, inspect.stack()[0].function)
    except Exception as e:
        error_message = str(e)        
...
def test_recordedOutOfTimeDecision_ac3(json, M1_bronze, M3_bronze):
    try:
        ...
        if test_df != None:
            ...
            if ac.count() != 0:
                return TestResult("recordedOutOfTimeDecision", "FAIL", f"recordedOutOfTimeDecision acceptance criteria failed: {str(ac.count())} cases have been found where M1.OutOfTimeIssue != 1 + M3.Outcome != 0 and recordedOutOfTimeDecision is not null", test_from_state, inspect.stack()[0].function)
            else:
                return TestResult("recordedOutOfTimeDecision", "PASS", f"recordedOutOfTimeDecision acceptance criteria passed, all cases where M1.OutOfTimeIssue != 1 + M3.Outcome != 0 have recordedOutOfTimeDecision is not null", test_from_state, inspect.stack()[0].function)
        else:
            return TestResult("recordedOutOfTimeDecision", "FAIL",f"Failed to Setup Data for Test - recordedOutOfTimeDecision does not exist in the payload", test_from_state, inspect.stack()[0].function)
    except Exception as e:
        error_message = str(e)        
