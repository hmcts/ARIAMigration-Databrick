...
def hearingResponse(silver_m1, silver_m3, silver_m6):
    ...
    df_stg = (
        silver_m3.filter(F.col("CaseStatus").isin([37, 38]))
        .withColumn("rn", F.row_number().over(window))
        .filter(F.col("rn") == 1)
        .drop(F.col("rn"))
    )

    m3_df = df_stg.withColumn(
        "CourtClerkFull",
        when(
            (col("CourtClerk_Surname").isNotNull()) & (col("CourtClerk_Surname") != ""),
            concat_ws(
                " ",
                col("CourtClerk_Surname"),
                col("CourtClerk_Forenames"),
                when(
                    (col("CourtClerk_Title").isNotNull())
                    & (col("CourtClerk_Title") != ""),
                    concat(lit("("), col("CourtClerk_Title"), lit(")")),
                ).otherwise(lit(None)),
            ),
        ),
    ).withColumn(
    ...
    stg_m6 = (
        silver_m6.withColumn(
            "Transformed_Required",
            when(F.col("Required") == "0", lit("Not Required")).when(
                F.col("Required") == "1", lit("Required")
            ),
        )
        .withColumn(
            "ConcatJudgeDetails",
            concat_ws(
                ...
                col("Judge_Title"),
                when(col("Judge_Title").isNotNull(), ")"),
                when(col("Transformed_Required").isNotNull(), ":"),
                col("Transformed_Required"),
            ),
        )
        .groupBy("CaseNo")
        .agg(
            concat_ws("\n", collect_list(col("ConcatJudgeDetails"))).alias(
                "ConcatJudgeDetails_List"
            ),
        )
    )

    final_df = (
        silver_m1.join(m3_df, ["CaseNo"], "left")
        .join(stg_m6, ["CaseNo"], "left")
        .withColumn("CaseNo", trim(col("CaseNo")))
        .withColumn(
            "Hearing Centre",
            when(col("HearingCentre").isNull(), "N/A").otherwise(
                col("HearingCentre")
            ),  # ListedCentre
        )
        .withColumn(
            "Hearing Date",
            when(col("HearingDate").isNull(), "N/A").otherwise(
                date_format(col("HearingDate"), "yyyy-MM-dd")
            ),  # KeyDate
        )
        .withColumn(
            "Hearing Type",
        ...
        .withColumn(
            "Court", when(col("CourtName").isNull(), "N/A").otherwise(col("CourtName"))
        )
        .withColumn(
            "List Type",
        ...
        .withColumn(
            "List Start Time",
            when(col("StartTime").isNull(), "N/A").otherwise(
                date_format(col("StartTime"), "HH:mm:ss")
            ),
        )
        .withColumn(
            "Judge First Tier",
            when(
                coalesce(
                    col("Judge1FT_Surname"),
                    col("Judge2FT_Surname"),
                    col("Judge3FT_Surname"),
                ).isNotNull(),
                trim(
                    concat_ws(
                        " ",
                        when(
                            col("Judge1FT_Surname").isNotNull(),
                            concat_ws(
                                " ",
                                col("Judge1FT_Surname"),
                                col("Judge1FT_Forenames"),
                                when(
                                    col("Judge1FT_Title").isNotNull()
                                    & (col("Judge1FT_Title") != ""),
                                    concat(lit("("), col("Judge1FT_Title"), lit(")")),
                                ).otherwise(lit("")),
                            ),
                        ).otherwise(lit("")),
                        when(
                            col("Judge2FT_Surname").isNotNull(),
                            concat_ws(
                                " ",
                                col("Judge2FT_Surname"),
                                col("Judge2FT_Forenames"),
                                when(
                                    col("Judge2FT_Title").isNotNull()
                                    & (col("Judge2FT_Title") != ""),
                                    concat(lit("("), col("Judge2FT_Title"), lit(")")),
                                ).otherwise(lit("")),
                            ),
                        ).otherwise(lit("")),
                        when(
                            col("Judge3FT_Surname").isNotNull(),
                            concat_ws(
                                " ",
                                col("Judge3FT_Surname"),
                                col("Judge3FT_Forenames"),
                                when(
                                    col("Judge3FT_Title").isNotNull()
                                    & (col("Judge3FT_Title") != ""),
                                    concat(lit("("), col("Judge3FT_Title"), lit(")")),
                                ).otherwise(lit("")),
                            ),
                        ).otherwise(lit("")),
                    )
        ...
        .withColumn(
            "Estimated Duration",
            when(col("TimeEstimate").isNull(), "N/A").otherwise(
                col("TimeEstimate").cast("string")
            ),
        )
        .withColumn(
            "Required/Incompatible Judicial Officers",
            when(
                col("ConcatJudgeDetails_List").isNotNull(),
                concat(lit("\n"), col("ConcatJudgeDetails_List")),
            ),
        )
        .withColumn("Notes", when(col("Notes").isNull(), "N/A").otherwise(col("Notes")))
        .withColumn(
            ...
            substring(
                concat(
                    ...
                    lit("\nHearing Centre: "),
                    coalesce(col("Hearing Centre"), lit("N/A")),
                    lit("\nHearing Date: "),
                    coalesce(col("Hearing Date"), lit("N/A")),
                    lit("\nHearing Type: "),
                    coalesce(col("Hearing Type"), lit("N/A")),
                    lit("\nCourt: "),
                    coalesce(col("Court"), lit("N/A")),
                    lit("\nList Type: "),
                    coalesce(col("ListType"), lit("N/A")),
                    lit("\nList Start Time: "),
                    coalesce(col("List Start Time"), lit("N/A")),
                    lit("\nJudge First Tier: "),
                    coalesce(col("Judge First Tier"), lit("")),
                    lit("\nCourt Clerk / Usher: "),
                    coalesce(
                        nullif(concat_ws(", ", col("CourtClerkFull")), lit("")),
                        lit("N/A"),
                    ),
                    lit("\nStart Time: "),
                    coalesce(col("Start Time"), lit("N/A")),
                    lit("\nEstimated Duration: "),
                    coalesce(col("Estimated Duration"), lit("N/A")),
                    lit("\nRequired/Incompatible Judicial Officers: "),
                    coalesce(col("Required/Incompatible Judicial Officers"), lit("")),
                    lit("\nNotes: "),
                    coalesce(col("Notes"), lit("N/A")),
                ),
                1,
                2000,
            ),
    ...
    df_hearingResponse = (
        ...
        .withColumn(
            ...
            lit(
                "Granted - This is a migrated ARIA case. Please refer to the documents."
            ),
        )
        .withColumn(
        ...
        .withColumn(
            ...
            when(col("m1.CourtPreference").isin([1, 2]), lit("Granted")).otherwise(
                lit(None)
            ),
        )
        .withColumn(
    ...
    df_audit_hearingResponse = (
        ...
        .select(
            ...
            array(struct(col("m1.InCamera"))).alias(
                "isInCameraCourtAllowed_inputValues"
            ),
            col("hr.isInCameraCourtAllowed").alias("isInCameraCourtAllowed_value"),
            lit("Yes").alias("isInCameraCourtAllowed_Transformed"),
            array(struct(lit("InCamera"))).alias(
                "inCameraCourtTribunalResponse_inputFields"
            ),
            array(struct(col("m1.InCamera"))).alias(
                "inCameraCourtTribunalResponse_inputValues"
            ),
            col("hr.inCameraCourtTribunalResponse").alias(
            ...
            array(struct(col("m1.InCamera"))).alias(
                "inCameraCourtDecisionForDisplay_inputValues"
            ),
            col("hr.inCameraCourtDecisionForDisplay").alias(
                "inCameraCourtDecisionForDisplay_Transformed"
            ),
            array(struct(lit("ListTypeId"))).alias(
                "isAppealSuitableToFloat_inputFields"
            ),
            array(struct(col("m3.ListTypeId"))).alias(
                "isAppealSuitableToFloat_inputValues"
            ),
            col("hr.isAppealSuitableToFloat").alias("isAppealSuitableToFloat_value"),
            lit("Yes").alias("isAppealSuitableToFloat_Transformed"),
            # isSingleSexCourtAllowed
            array(struct(lit("CourtPreference"))).alias(
                "isSingleSexCourtAllowed_inputFields"
            ),
            array(struct(col("m1.CourtPreference"))).alias(
                "isSingleSexCourtAllowed_inputValues"
            ),
            ...
            array(struct(col("m1.CourtPreference"))).alias(
                "singleSexCourtTribunalResponse_inputValues"
            ),
            col("hr.singleSexCourtTribunalResponse").alias(
            ...
            array(struct(col("m1.CourtPreference"))).alias(
                "singleSexCourtDecisionForDisplay_inputValues"
            ),
            col("hr.singleSexCourtDecisionForDisplay").alias(
            ...
            array(struct(*common_inputValues)).alias(
                "multimediaTribunalResponse_inputValues"
            ),
            col("hr.multimediaTribunalResponse").alias(
            ...
            array(struct(*common_inputValues)).alias(
                "multimediaDecisionForDisplay_inputValues"
            ),
            col("hr.multimediaDecisionForDisplay").alias(
            ...
            array(struct(*common_inputValues)).alias(
                "isVulnerabilitiesAllowed_inputValues"
            ),
            ...
            array(struct(*common_inputValues)).alias(
                "vulnerabilitiesTribunalResponse_inputValues"
            ),
            col("hr.vulnerabilitiesTribunalResponse").alias(
            ...
            array(struct(*common_inputValues)).alias(
                "vulnerabilitiesDecisionForDisplay_inputValues"
            ),
            col("hr.vulnerabilitiesDecisionForDisplay").alias(
            ...
            array(struct(*common_inputValues)).alias(
                "isRemoteHearingAllowed_inputValues"
            ),
            ...
            array(struct(*common_inputValues)).alias(
                "remoteVideoCallTribunalResponse_inputValues"
            ),
            col("hr.remoteVideoCallTribunalResponse").alias(
            ...
            array(struct(*common_inputValues)).alias(
                "isAdditionalAdjustmentsAllowed_inputValues"
            ),
            col("hr.isAdditionalAdjustmentsAllowed").alias(
            ...
            array(struct(*common_inputValues)).alias(
                "additionalTribunalResponse_inputValues"
            ),
            col("hr.additionalTribunalResponse").alias(
            ...
            array(struct(*common_inputValues)).alias(
                "otherDecisionForDisplay_inputValues"
            ),
            ...
            array(struct(*common_inputValues)).alias(
                "isAdditionalInstructionAllowed_inputValues"
            ),
            col("hr.isAdditionalInstructionAllowed").alias(
            ...
            array(
                ...
                struct(
                    ...
                    array(
                        ...
                        lit("Judge1FT_Forenames"),
                        lit("Judge1FT_Title"),
                        lit("Judge2FT_Surname"),
                        lit("Judge2FT_Forenames"),
                        lit("Judge2FT_Title"),
                        lit("Judge3FT_Surname"),
                        lit("Judge3FT_Forenames"),
                        lit("Judge3FT_Title"),
                ...
                struct(
                    ...
                    array(
                        lit("CourtClerk_Surname"),
                        lit("CourtClerk_Forenames"),
                        lit("CourtClerk_Title"),
                    ).alias("source_columns"),
                ),
                struct(
            ...
            array(
                struct(
                    lit("Hearing Centre").alias("field"),
                    col("f.`Hearing Centre`").cast("string").alias("value"),
                ),
                struct(
                    lit("Hearing Date").alias("field"),
                    col("f.`Hearing Date`").cast("string").alias("value"),
                ),
                struct(
                    lit("Hearing Type").alias("field"),
                    col("f.`Hearing Type`").cast("string").alias("value"),
                ),
                struct(
                    lit("Court").alias("field"),
                    col("f.`Court`").cast("string").alias("value"),
                ),
                struct(
                    lit("List Type").alias("field"),
                    col("f.`List Type`").cast("string").alias("value"),
                ),
                struct(
                    lit("List Start Time").alias("field"),
                    col("f.`List Start Time`").cast("string").alias("value"),
                ),
                struct(
                    lit("Judge First Tier").alias("field"),
                    col("f.`Judge First Tier`").cast("string").alias("value"),
                ),
                struct(
                    lit("Start Time").alias("field"),
                    col("f.`Start Time`").cast("string").alias("value"),
                ),
                struct(
                    lit("Estimated Duration").alias("field"),
                    col("f.`Estimated Duration`").cast("string").alias("value"),
                ),
                struct(
                    lit("Required/Incompatible Judicial Officers").alias("field"),
                    col("f.`Required/Incompatible Judicial Officers`")
                    .cast("string")
                    .alias("value"),
                ),
                struct(
                    lit("Notes").alias("field"),
                    col("f.`Notes`").cast("string").alias("value"),
                ),
                struct(
                    lit("Court Clerk / Usher").alias("field"),
                    col("f.CourtClerkFull").cast("string").alias("value"),
                ),
                struct(
                    lit("dv_representation").alias("field"),
                    col("m1.`dv_representation`").cast("string").alias("value"),
                ),
...
def hearingDetails(silver_m1, silver_m3, bronze_listing_location):
    ...
    listing_location_list = array(
        [
            struct(
                lit(row.locationCode).cast("string").alias("code"),
                lit(row.locationLabel).cast("string").alias("label"),
            )
    ...
    silver_m3_filtered_casestatus = (
        ...
        .withColumn(
            "start_time_str",
            F.when(F.col("m3.StartTime").isNull(), F.lit("00:00:00.000")).otherwise(
        ...
        .withColumn(
            ...
            F.col("location.listCaseHearingCentreAddress"),
        )
    )
    ...
    hearingChannelListItems = array(
        struct(
            lit("INTER").cast("string").alias("code"),
            lit("In Person").cast("string").alias("label"),
        ),
        struct(
            lit("NA").cast("string").alias("code"),
            lit("Not in Attendance").cast("string").alias("label"),
        ),
        struct(
            lit("ONPPRS").cast("string").alias("code"),
            lit("On The Papers").cast("string").alias("label"),
        ),
        struct(
            lit("TEL").cast("string").alias("code"),
            lit("Telephone").cast("string").alias("label"),
        ),
        struct(
            lit("VID").cast("string").alias("code"),
            lit("Video").cast("string").alias("label"),
        ),
    )
    ...
    df_hearingDetails = (
        ...
        .withColumn(
            ...
            map_from_arrays(
                lit([]).cast("array<string>"), lit([]).cast("array<string>")
    ...
    df_audit_hearingDetails = (
        ...
        .select(
            ...
            # listCaseHearingLength
            array(struct(lit("CaseNo"), lit("TimeEstimate"))).alias(
                "listCaseHearingLength_inputFields"
            ),
            array(struct(col("CaseNo"), col("TimeEstimate"))).alias(
            ...
            # listCaseHearingDate
            array(struct(lit("CaseNo"), lit("HearingDate"), lit("StartTime"))).alias(
                "listCaseHearingDate_inputFields"
            ),
            array(struct(col("CaseNo"), col("HearingDate"), col("StartTime"))).alias(
            ...
            array(
                struct(
                    ...
                    lit("ListedCentre"),
                    lit("HearingCentre"),
                    lit("listCaseHearingCentre"),
                )
            ).alias("listCaseHearingCentre_inputFields"),
            array(
                struct(
                    col("CaseNo"),
                    col("location.ListedCentre"),
                    col("HearingCentre"),
                    col("location.listCaseHearingCentre"),
                )
            ...
            array(
                struct(
                    ...
                    lit("ListedCentre"),
                    lit("HearingCentre"),
                    lit("listCaseHearingCentreAddress"),
                )
            ).alias("listCaseHearingCentreAddress_inputFields"),
            array(
                struct(
                    col("CaseNo"),
                    col("location.ListedCentre"),
                    col("HearingCentre"),
                    col("location.listCaseHearingCentreAddress"),
                )
            ).alias("listCaseHearingCentreAddress_inputValues"),
            col("hd.listCaseHearingCentreAddress").alias(
...
def generalDefault(silver_m1):
    ...
    generalDefault_df = generalDefault_df.withColumn(
        "reviewedHearingRequirements", lit("Yes")
    )
