...
def bronze_interpreter_languages_test_data(spark):
    ...
    data = [
        ...
        (8, "Manual", "spokenLanguageInterpreter", None, None, "Yes", "Additional Manual Entry"),        # Spoken Language Additional Manual Entry
        (9, "Manual", "signLanguageInterpreter", None, None, "Yes", "Additional Manual Sign Entry")      # Sign Language Additional Manual Entry
    ]
...
class TestListingState():
    ...
    def test_hearing_requirements_interpreter_language_fields(self, spark, bronze_interpreter_languages_test_data):
        m1_data = [
            ...
            ("4", "AIP", "FTPA", None, 0, 0, 0, 7),  # LanguageCode 7 - Sign Language Manual Entry
            ...
            ("13", "AIP", "FT", None, 0, 0, 0, 7),   # For m3 conditional tests - Additional Language Sign Manual + Spoken Manual
            ("14", "AIP", "FT", None, 0, 0, 0, 7)    # For m3 conditional tests - Additional Language Sign Manual + Sign Manual
        ]
        ...
        m3_data = [
            ...
            ("12", 1, 37, 0, 8),  # Additional Spoken Manual Language (to Spoken Manual + Spoken Manual)
            ("13", 1, 37, 0, 8),  # Additional Spoken Manual Language (to Sign Manual + Spoken Manual)
            ("14", 1, 37, 0, 9),  # Additional Sign Manual Language (to Sign Manual + Sign Manual)
        ]
    ...
    def test_hearing_requirements_m3_conditional_fields(self, spark, bronze_interpreter_languages_test_data):
        m1_data = [
            ("1", "AIP", "FT", None, 0, 0, 0, 1),   # For m3 conditional tests - CaseStatus = 37 AND Outcome = 0
            ("2", "AIP", "FT", None, 0, 0, 0, 1),   # For m3 conditional tests - CaseStatus = 37 AND Outcome = 27
            ("3", "AIP", "FT", None, 0, 0, 0, 1),   # For m3 conditional tests - CaseStatus = 37 AND Outcome = 37
            ("4", "AIP", "FT", None, 0, 0, 0, 1),   # For m3 conditional tests - CaseStatus = 37 AND Outcome = 39
            ("5", "AIP", "FT", None, 0, 0, 0, 1),   # For m3 conditional tests - CaseStatus = 37 AND Outcome = 40
            ("6", "AIP", "FT", None, 0, 0, 0, 1),   # For m3 conditional tests - CaseStatus = 37 AND Outcome = 50
            ("7", "AIP", "FT", None, 0, 0, 0, 1),   # For m3 conditional tests - CaseStatus = 38 AND Outcome = 0
            ("8", "AIP", "FT", None, 0, 0, 0, 1),   # For m3 conditional tests - CaseStatus = 38 AND Outcome = 27
            ("9", "AIP", "FT", None, 0, 0, 0, 1),   # For m3 conditional tests - CaseStatus = 38 AND Outcome = 37
            ("10", "AIP", "FT", None, 0, 0, 0, 1),  # For m3 conditional tests - CaseStatus = 38 AND Outcome = 39
            ("11", "AIP", "FT", None, 0, 0, 0, 1),  # For m3 conditional tests - CaseStatus = 38 AND Outcome = 40
            ("12", "AIP", "FT", None, 0, 0, 0, 1),  # For m3 conditional tests - CaseStatus = 38 AND Outcome = 50
            ("13", "AIP", "FT", None, 0, 0, 0, 1),  # For m3 conditional tests - CaseStatus = 26 AND Outcome = 40
            ("14", "AIP", "FT", None, 0, 0, 0, 1),  # For m3 conditional tests - CaseStatus = 26 AND Outcome = 52
            ("15", "AIP", "FT", None, 0, 0, 0, 1),  # For m3 conditional tests - Not Matching CaseStatus: CaseStatus = 39 AND Outcome = 0
            ("16", "AIP", "FT", None, 0, 0, 0, 1),  # For m3 conditional tests - Not Matching Outcome: CaseStatus = 37 AND Outcome = 52
            ("17", "AIP", "FT", None, 0, 0, 0, 1),  # For m3 conditional tests - Not Matching Outcome: CaseStatus = 26 AND Outcome = 0
            ("18", "AIP", "FT", None, 0, 0, 0, 1),  # For m3 conditional tests - Not Matching Outcome: CaseStatus = 0 AND Outcome = 1
            ("19", "AIP", "FT", None, 0, 0, 0, 1),  # For m3 conditional tests - StatusId Check (Many Statuses)
            ("20", "AIP", "FT", None, 0, 0, 0, 1)   # For m3 conditional tests - StatusId Check (CaseStatus and Outcome)
        ]
