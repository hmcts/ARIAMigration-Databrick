...
def documents(silver_m1):
    ...
    documents_audit = (
        ...
        .select(
            "audit.*",
            array(struct(lit("finalDecisionAndReasonsDocuments"))).alias(
                "finalDecisionAndReasonsDocuments_inputFields"
            ),
            array(struct(lit("null"))).alias(
                "finalDecisionAndReasonsDocumentss_inputValues"
            ),
            col("finalDecisionAndReasonsDocuments").alias(
...
def substantiveDecision(silver_m1, silver_m3):
    ...
    decision_ts = coalesce(
        ...
        F.to_timestamp(
            col("m3.DecisionDate"), "yyyy-MM-dd'T'HH:mm:ss.SSSX"
        ),  # e.g., Z or +01
    )
    ...
    substantiveDecision_audit = (
        ...
        .select(
            ...
            # ----- appealDecision -----
            array(struct(lit("CaseStatus"), lit("Outcome"))).alias(
                "appealDecision_inputFields"
            ),
            array(struct(col("CaseStatus"), col("Outcome"))).alias(
                "appealDecision_inputValues"
            ),
    ...
    hearingActuals_df = (
        ...
        .withColumn(
            ...
            F.create_map(
                ...
                F.lit("hours"),
                F.when(
                    col("HearingDuration").isNull(), F.lit(None).cast("string")
                ).otherwise(
                    F.floor(col("HearingDuration").cast("int") / 60).cast("string")
                ),
                # key: "minutes", value: minutes calculation
                F.lit("minutes"),
                F.when(col("HearingDuration").isNull(), F.lit(None).cast("string"))
                .otherwise(F.col("HearingDuration").cast("int") % 60)
                .cast("string"),
            ),
        )
        .withColumn(
        ...
        .select(col("CaseNo"), col("actualCaseHearingLength"), col("attendingJudge"))
    )

    hearingActuals_audit = (
        hearingActuals_df.alias("ha")
        .join(silver_m3_max_statusid.alias("m3"), on=["CaseNo"], how="left")
        .select(
            col("CaseNo"),
            array(struct(lit("HearingDuration"), lit("Outcome"))).alias(
                "actualCaseHearingLength_inputFields"
            ),
            array(struct(col("HearingDuration"), col("Outcome"))).alias(
                "actualCaseHearingLength_inputValues"
            ),
            ...
            ).alias("attendingJudge_inputFields"),
            array(
                struct(
                    col("Adj_Determination_Title"),
                    col("Adj_Determination_Forenames"),
...
def ftpa(silver_m1, silver_m2, silver_m3, silver_c):
    ...
    ftpa_df = ftpa_df.select(
        ...
        # col("dv_addressInUk"),
        date_format(
            when(is_detained_or_in_uk, date_add(col("DecisionDate"), 14)).when(
                ~is_detained_or_in_uk, date_add(col("DecisionDate"), 28)
            ),
            "yyyy-MM-dd",
...
def generalDefault(silver_m1):
    ...
    general_df = general_df.withColumn("appealDecisionAvailable", lit("Yes"))
