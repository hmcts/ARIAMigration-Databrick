...
with patch.dict(os.environ, {"ENVIRONMENT": "sbox", "LZ_KEY": "test0"}), \
...
def _build_trigger_mocks():
    ...
    mock_file = MagicMock()
    mock_temp_file_ctx = MagicMock()
    mock_temp_file_ctx.__enter__ = MagicMock(return_value=mock_file)
    mock_temp_file_ctx.__exit__ = MagicMock(return_value=False)

    mock_idempotency_blob = AsyncMock()
    mock_idempotency_blob.exists.return_value = False

    mock_ccd_mapping_blob = MagicMock()

    mock_idempotency_container = MagicMock()
    mock_idempotency_container.get_blob_client.return_value = mock_idempotency_blob

    mock_ccd_container = MagicMock()
    mock_ccd_container.get_blob_client.return_value = mock_ccd_mapping_blob

    mock_blob_svc = MagicMock()
    mock_blob_svc.close = AsyncMock()
    mock_blob_svc.get_container_client.side_effect = (
        lambda name: mock_idempotency_container if name == "af-idempotency" else mock_ccd_container
    )
    ...
    return {
        "file": mock_file,
        "temp_file_ctx": mock_temp_file_ctx,
        "blob_svc": mock_blob_svc,
        "producer": mock_producer,
        "kv": mock_kv,
        "ccd_mapping_blob": mock_ccd_mapping_blob,
    }


# eventhub_trigger_active: case-linking file write and blob upload tests
...
def test_eventhub_trigger_writes_case_link_on_success(
        mock_temp_file, mock_process_case,
        mock_credential, mock_secret_client,
        mock_blob_service, mock_eh_producer):
    mocks = _build_trigger_mocks()
    mock_temp_file.return_value = mocks["temp_file_ctx"]
    ...
    expected = json.dumps({"ARIACaseNo": "CASE123", "CCDCaseNo": "9876543210"})
    mocks["file"].write.assert_called_once_with(expected)
...
@patch("AzureFunctions.ACTIVE.active_ccd.function_app.process_case")
@patch("tempfile.TemporaryFile")
def test_eventhub_trigger_no_case_link_written_on_error(
        mock_temp_file, mock_process_case,
        mock_credential, mock_secret_client,
        mock_blob_service, mock_eh_producer):
    mocks = _build_trigger_mocks()
    ...
    mocks["file"].write.assert_not_called()
...
@patch("AzureFunctions.ACTIVE.active_ccd.function_app.process_case")
@patch("tempfile.TemporaryFile")
def test_eventhub_trigger_uploads_mapping_blob(
        mock_temp_file, mock_process_case,
        mock_credential, mock_secret_client,
        mock_blob_service, mock_eh_producer):
    mocks = _build_trigger_mocks()
    ...
    mocks["ccd_mapping_blob"].upload_blob.assert_called_once_with(mocks["file"])
