...
@pytest.fixture(scope="session")
def dq_checks():
    return listing_dq_rules.listingDQRules().add_checks({})
...
class TestListingDQChecks():
    ...
    def test_valid_appellantInterpreterLanguageCategory_check(self, spark, dq_checks):
        df = spark.createDataFrame([
            ("1", 1, 0, 0, None, None, None),         # No languages - valid
            ("2", 1, None, None, None, None, None),   # No languages with nulls - valid
            ("3", 1, 1, 0, ["a"], "a", None),         # 1 language, no additional - valid
            ("4", 1, 1, None, ["b"], "b", None),      # 1 language, additional is null - valid
            ("5", 1, 0, 2, ["c"], None, "c"),         # Additional language only - valid
            ("6", 1, None, 2, ["c"], None, "c"),      # Additional language only, primary null - valid
            ("7", 1, 0, 0, ["d"], "d", "d"),          # Category is set despite no languages, with valid inputs set - invalid
            ("8", 1, None, None, ["e"], None, None),  # Category is set with null languages - invalid
            ("9", 1, 0, 0, None, "f", "f"),           # No languages, even with valid inputs set - valid (as valid inputs are dropped from final_df)
            ("10", 1, 3, 4, ["g", "h"], "g", "h"),    # Both language and additional, set as expected - valid
            ("11", 1, 3, 4, None, "g", "h"),          # Both languages but no categories set - invalid
            ("12", 1, 3, 4, ["g", "h"], None, None),  # Both languages with categories set, but unable to confirm due to missing valid inputs - invalid NULL
            ("13", 1, 5, 6, ["i", "j"], "k", "l"),    # Both languages with categories set but differing valid inputs - invalid
            ("14", 1, 5, 6, ["i"], "i", "i"),         # Both languages but 1 category entry when duplicate in valid inputs - valid
            ("15", 1, 5, 0, ["i"], "j", None),        # 1 language, but mismatch in valid inputs value - invalid
            ("16", 1, 5, 0, ["i"], None, "i"),        # 1 language, but mismatch of valid inputs between primary and additional - invalid
            ("17", 1, 0, 6, ["j"], "j", None),        # 1 additional language, but mismatch in valid inputs between primary and additional - invalid
            ("18", 1, 0, 6, ["j"], None, "i"),        # 1 additional language, but mismatch of valid inputs value - invalid
            ("19", 1, 1, 0, [], None, None),          # LanguageId set but no category - invalid
            ("20", 2, 1, 0, ["a"], "a", None)         # Interpreter is not needed but category exists - invalid
        ...
        assert df_checks[14][0] is not True and df_checks[15][0] is not True and df_checks[16][0] is not True and df_checks[17][0] is not True and df_checks[18][0] is not True and df_checks[19][0] is not True
    ...
    def test_valid_appellantInterpreterSpokenLanguage(self, spark, dq_checks):
        ...
        assert df_checks[8][0] is True and df_checks[9][0] is True
        assert df_checks[10][0] is not True and df_checks[11][0] is not True
