...
# %sql
# select distinct Priority from hive_metastore.ariadm_arm_appeals.raw_category
...
# %sql
# select segment, count(*) from ariadm_arm_fta.stg_appeals_filtered group by segment

# COMMAND ----------

# %sql
# select CaseStatusCategory, count(*) from ariadm_arm_fta.stg_appealcasestatus_filtered group by CaseStatusCategory
...
# DBTITLE 1,Red flag logic
# %sql
# WITH CTE AS (
#     SELECT 
#         CaseNo, 
#         CASE 
#             WHEN CostOrderAppliedFor = 'enabled' THEN 'REC' 
#             ELSE '' 
#         END AS CostOrderAppliedFor
#     FROM hive_metastore.ariadm_arm_appeals.silver_status_detail
#     WHERE CaseStatus IN (27, 28, 29, 30, 31, 32, 33, 34)
#     GROUP BY CaseNo, CostOrderAppliedFor
# ), cte_rec AS (
#     SELECT 
#         CaseNo,
#         CASE 
#             WHEN CostOrderAppliedFor = 'REC' THEN 'REC' 
#             ELSE '' 
#         END AS CostOrderAppliedFor 
#     FROM CTE
#     WHERE CostOrderAppliedFor = 'REC' 
#     GROUP BY CaseNo, CostOrderAppliedFor
# ), Flag_List AS (
#     SELECT 
#         ac.CaseNo, 
...
#                 SELECT 1 
#                 FROM hive_metastore.ariadm_arm_appeals.silver_transaction_detail t 
...
#             ELSE ''
#         END AS Fee_Flag
#     FROM hive_metastore.ariadm_arm_appeals.silver_transaction_detail d
# ), cte_feeflagordered AS (
#     SELECT *,
#         CASE Fee_Flag
#             WHEN 'FEE REMISSION' THEN 1
#             WHEN 'FEE EXEMPT' THEN 2
#             WHEN 'FEE PAID' THEN 3
#             WHEN 'FEE DUE' THEN 4
#             ELSE 0
#         END AS Fee_Flag_num
#     FROM cte_feeflag
# ), max_fee_flag_num AS (
#     SELECT 
#         MAX(Fee_Flag_num) AS max_Fee_Flag_num,
#         caseNo 
...
#         Fee_flag, 
#         Fee_Flag_num 
#     FROM cte_feeflagordered cfo 
#     JOIN max_fee_flag_num mf 
#         ON cfo.caseNo = mf.caseNo 
#         AND cfo.Fee_Flag_num = mf.max_Fee_Flag_num
#     WHERE max_Fee_Flag_num != 0
# )
# SELECT
#     a.CaseNo,
#     CONCAT(
...
#         END,
#         CASE 
#             WHEN Detained IS NULL THEN '     ' 
#             ELSE CONCAT('*', Detained, '*') 
#         END,
#         CASE 
#             WHEN CostOrderAppliedFor = 'REC' THEN '*REC*' 
#             ELSE '' 
...
# %sql
# SELECT * FROM hive_metastore.ariadm_arm_fta.stg_statichtml_data
# where CaseNo = "IM/00021/2003"

# COMMAND ----------

# DBTITLE 1,Red PySpark
# # from pyspark.sql.functions import when, col, collect_list, struct, sort_array, concat_ws, size, expr, coalesce, lit, max as spark_max

# # Read tables from Hive
# df_status = spark.table("hive_metastore.ariadm_arm_fta.silver_status_detail")
# df_category = spark.table("hive_metastore.ariadm_arm_fta.silver_appealcategory_detail")
# df_transaction = spark.table("hive_metastore.ariadm_arm_fta.silver_transaction_detail")
...
# # CTE for CostOrderAppliedFor
# cte = (df_status
#     .filter(col("CaseStatus").isin([27, 28, 29, 30, 31, 32, 33, 34]))
#     .select("CaseNo", when(col("CostOrderAppliedFor") == 'enabled', 'REC').otherwise('').alias("CostOrderAppliedFor"))
#     .distinct())

# cte_rec = cte.filter(col("CostOrderAppliedFor") == 'REC').select("CaseNo", "CostOrderAppliedFor")

# # CTE for Flag_List
# flag_list = (df_category
#     .join(df_status, "CaseNo")
#     .filter(col("CaseStatus") == 35)
#     .groupby("CaseNo")
#     .agg(sort_array(collect_list(struct(col("Priority"), concat_ws('', lit('*'), col("Flag"), lit('*')))), asc=False).alias("sorted_flags")))
...
# df_transaction_filtered = df_transaction.filter(col("TransactionTypeId") == 1).groupby("CaseNo").agg(count("*").alias("txn_count"))

# cte_feeflag = (df_transaction
...
# max_fee_flag_num = cte_feeflagordered.groupby("CaseNo").agg(max("Fee_Flag_num").alias("max_Fee_Flag_num"))
...
#                 .otherwise('     '),
#             when(col("Detained").isNull(), '     ').otherwise(concat_ws("", lit('*'), col("Detained"), lit('*'))),
#             when(col("CostOrderAppliedFor") == 'REC', '*REC*').otherwise('')
#         ).alias("flag1"),
#         concat_ws("",
...
# # display(final_df.filter(col("CaseNo") == lit("IM/00021/2003")))
...
# df_transaction_details = spark.table("hive_metastore.ariadm_arm_fta.silver_transaction_detail")
# df_history_details = spark.table("hive_metastore.ariadm_arm_fta.silver_history_detail")
# df_status_details = spark.table("hive_metastore.ariadm_arm_fta.silver_status_detail")
# df_link_details = spark.table("hive_metastore.ariadm_arm_fta.silver_link_detail")
...
#                                                   .select("a.CaseNo", "c.*", "flag1", "flag2", expr("regexp_replace(flag3, ',', '')").alias("flag3"))



# display(df_case.filter(col("CaseNo") == lit("IM/00021/2003")))
# display(final_df.filter(col("CaseNo") == lit("IM/00021/2003")))
# display(df_stg_static_html_data.filter(col("CaseNo") == lit("IM/00021/2003")))
# display(df_stg_static_html_final_data.filter(col("a.CaseNo") == lit("IM/00021/2003")))
...
# The 'FEE PAID' flag will be displayed when the the total value of payments made to the case is equal to or greater than the total value of fees applied to the case and a receipt transaction exists.

# Same Logic as s used for this field

# The 'FEE DUE' flag will be displayed when the balance is greater that zero and Appeal Fee was due

# The 'Appeal Fee' (TransactionTypeId = 1) to be present on the case

# The 'FEE EXEMPT' flag will be displayed if the Fee Event is Fee Exempt and the Balance is zero

# Fee exempt is TransactionTypeId = 17 in same as above

# The 'FEE REMISSION' flag will be displayed when an Appeal Fee Remission has been applied and the balance is zero.

# Fee exempt is TransactionTypeId = 5 in same as above
...
#     description = "The stg_appeals_unified Delta Live staging table consolidates all silver data, including HTML, JSON, and A360 content, along with static and status details-derived data for HTML."
...
# %sql
# select count(*), A360BatchId from hive_metastore.ariadm_arm_fta.gold_appeals_with_html
...
# cte = (df_status_details
#     .filter(col("CaseStatus").isin([27, 28, 29, 30, 31, 32, 33, 34]))
#     .select("CaseNo", when(col("CostOrderAppliedFor") == 'enabled', 'REC').otherwise(lit(None)).alias("CostOrderAppliedFor"))
#     .distinct())

# cte_rec = cte.filter(col("CostOrderAppliedFor") == 'REC').select("CaseNo", "CostOrderAppliedFor").distinct()
...
# df_stg_static_html_final_data.display()
