...
@app.function_name("eventhub_trigger_appeals")
@app.event_hub_message_trigger(
    arg_name="appealsazeventhub",
    event_hub_name="evh-apl-pub-dev-uks-dlrm-01",
...
async def eventhub_trigger_appeals(appealsazeventhub: List[func.EventHubEvent]):
    logging.info(f"Processing a batch of {len(appealsazeventhub)} events")
    ...
    try:
        ...
        ev_dl_key = kv_client.get_secret("evh-apl-dl-uks-dlrm-01").value
        ev_ack_key = kv_client.get_secret("evh-apl-ack-uks-dlrm-01").value
        ...
        container_secret = kv_client.get_secret("ARIAFTA-SAS-TOKEN").value
        ...
        async with ContainerClient.from_container_url(container_url) as container_service_client, \
            ...
            tasks = [
                process_messages(event, container_service_client, "ARIAFTA/submission", dl_producer_client, ack_producer_client)
                for event in appealsazeventhub
            ]
...
async def process_messages(event, container_service_client, subdirectory, dl_producer_client, ack_producer_client):
    """Processes individual messages from Event Hub."""
    results = {
        "filename": None,
        "http_response": None,
        "timestamp": None,
    ...
    for attempt in range(retries):
        try:
            ...
            if not await blob_client.exists():
                ...
                results.update({
                    "http_response": 409,
                    "http_message": "Blob Already Exists",
