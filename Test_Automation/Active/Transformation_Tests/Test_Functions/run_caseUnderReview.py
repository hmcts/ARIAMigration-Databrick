import Test_Functions.CaseUnderReview_Tests as cur_tests
...
def run_all_tests(json_data, M1_bronze, M1_silver, M2_bronze, M3_bronze, M3_silver, M6_bronze, C, bhc, fields_to_exclude):
    ...
    test_df, test_data_setup =  cur_tests.test_default_mapping_init(json_data, M1_silver)
    ...
    # -- hearingResponse --
    test_data_setup = None
    test_df, test_data_setup =  cur_tests.test_hearingResponse_init(json_data, M3_silver, M6_bronze)
    if test_data_setup != True:
         all_test_results.append(test_data_setup)

    if test_df != None:
        all_test_results.append(cur_tests.test_hearingResponse(test_df))
