...
def generate_dashboard():
    ...
    Includes totals, correct DataTables mapping, pie chart, and compare filters:
    - only failures
    - only differences
    """
    ...
    for _, run in runs_df.iterrows():
        ...
        if run_dict.get("run_start_datetime") is not None:
            run_dict["run_start_datetime"] = run_dict["run_start_datetime"].strftime("%Y-%m-%d %H:%M:%S")
        if run_dict.get("run_end_datetime") is not None:
...
<div class="controls">
    ...
    <div class="checkbox-group">
        <input type="checkbox" id="onlyFailures">
        <label for="onlyFailures">Only failures</label>
    </div>

    <div class="checkbox-group">
        <input type="checkbox" id="onlyDifferences">
        <label for="onlyDifferences">Only differences</label>
    </div>
</div>

<div id="compareSummary" class="summary" style="display:none;"></div>
<div id="compareResults"></div>

<script>
let allResults = {all_results_json};
...
function compareRuns() {{
    let runA = $('#runA').val();
    let runB = $('#runB').val();
    let onlyFailures = $('#onlyFailures').is(':checked');
    let onlyDifferences = $('#onlyDifferences').is(':checked');
    ...
    let allKeys = Array.from(new Set([...Object.keys(mapA), ...Object.keys(mapB)])).sort();
    ...
    allKeys.forEach(key => {{
        let rowsA = mapA[key] || [{{ status: "MISSING", message: "" }}];
        let rowsB = mapB[key] || [{{ status: "MISSING", message: "" }}];
        let maxLen = Math.max(rowsA.length, rowsB.length);

        for (let i = 0; i < maxLen; i++) {{
            let rowA = rowsA[i] || {{ status: "MISSING", message: "" }};
            let rowB = rowsB[i] || {{ status: "MISSING", message: "" }};

            let statusA = rowA.status || "";
            let statusB = rowB.status || "";
            let messageA = rowA.message || "";
            let messageB = rowB.message || "";

            let parts = key.split("|");
            let testName = parts[0] || "";
            let testField = parts[1] || "";

            let isDifferent = statusA !== statusB || messageA !== messageB;
            let isFailure = statusA === "FAIL" || statusB === "FAIL";

            totalRows++;
            if (isDifferent) diffRows++;
            if (isFailure) failRows++;

            if (onlyDifferences && !isDifferent) {{
                continue;
            }}

            if (onlyFailures && !isFailure) {{
                continue;
            }}

            let rowClass = isDifferent ? "diff-row" : "match-row";
            ...
            html += `
                <tr class="${{rowClass}}">
                    ...
                    <td>${{testName}}</td>
                    <td>${{testField}}</td>
                    <td class="${{getStatusClass(statusA)}}">${{statusA}}</td>
                    <td class="${{getStatusClass(statusB)}}">${{statusB}}</td>
                    <td>${{messageA}}</td>
                    <td>${{messageB}}</td>
                </tr>
            `;
    ...
    $('#compareSummary').html(
        "<strong>Run A:</strong> " + runA +
        " &nbsp; | &nbsp; <strong>Run B:</strong> " + runB +
        " &nbsp; | &nbsp; <strong>Total compared rows:</strong> " + totalRows +
        " &nbsp; | &nbsp; <strong>Differences:</strong> " + diffRows +
        " &nbsp; | &nbsp; <strong>Failures:</strong> " + failRows +
        " &nbsp; | &nbsp; <strong>Rows shown:</strong> " + shownRows
    ).show();
...
$('#onlyFailures, #onlyDifferences').on('change', function() {{
    let runA = $('#runA').val();
    let runB = $('#runB').val();
    if (runA && runB && runA !== runB) {{
        compareRuns();
...
function downloadExcel() {{
    var wb = XLSX.utils.json_to_sheet(resultsData);
    var workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, wb, "Results");
    XLSX.writeFile(workbook, "run_{run_id}_results.xlsx");
}}
</script>
</body>
</html>
