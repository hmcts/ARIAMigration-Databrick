...
def appealType(silver_m1):
    conditions = (col("dv_representation").isin("LR", "AIP")) & (
        col("lu_appealType").isNotNull()
    )
    aip_condition = (col("dv_representation") == "AIP") & (
        col("lu_appealType").isNotNull()
    )

    df = silver_m1.select(
        col("CaseNo"),
        when(conditions, col("lu_appealType")).otherwise(None).alias("appealType"),
        when(conditions, col("lu_hmctsCaseCategory"))
        .otherwise(None)
        .alias("hmctsCaseCategory"),
        when(conditions, col("CaseNo")).otherwise(None).alias("appealReferenceNumber"),
        when(conditions, col("lu_appealTypeDescription"))
        .otherwise(None)
        .alias("appealTypeDescription"),
        when(conditions, col("lu_caseManagementCategory"))
        .otherwise(None)
        .alias("caseManagementCategory"),
        when(aip_condition, lit("Yes"))
        .otherwise(lit(None))
        .alias("isAppealReferenceNumberAvailable"),
        when(conditions, lit(""))
        .otherwise(lit(None))
        .alias("ccdReferenceNumberForDisplay"),
    ).distinct()
    ...
    df_audit = (
        ...
        .select(
            ...
            array(struct(*common_inputFields, lit("CaseNo"))).alias(
                "appealReferenceNumber_inputfields"
            ),
            array(struct(*common_inputValues, col("CaseNo"))).alias(
                "appealReferenceNumber_inputvalues"
            ),
            col("content.appealReferenceNumber"),
            lit("no").alias("appealReferenceNumber_Transformation"),
            # Audit hmctsCaseCategory
            array(struct(*common_inputFields, lit("lu_hmctsCaseCategory"))).alias(
                "hmctsCaseCategory_inputfields"
            ),
            array(struct(*common_inputValues, col("audit.lu_hmctsCaseCategory"))).alias(
                "hmctsCaseCategory_inputvalues"
            ),
            col("content.hmctsCaseCategory"),
            lit("yes").alias("hmctsCaseCategory_Transformation"),
            # Audit appealTypeDescription
            array(struct(*common_inputFields, lit("lu_appealTypeDescription"))).alias(
                "appealTypeDescription_inputFields"
            ),
            array(
                struct(*common_inputValues, col("audit.lu_appealTypeDescription"))
            ).alias("appealTypeDescription_inputValues"),
            col("content.appealTypeDescription"),
            lit("yes").alias("appealTypeDescription_Transformation"),
            # Audit caseManagementCategory
            array(struct(*common_inputFields, lit("lu_caseManagementCategory"))).alias(
                "caseManagementCategory_inputFields"
            ),
            array(
                struct(*common_inputValues, col("audit.lu_caseManagementCategory"))
            ).alias("caseManagementCategory_inputValues"),
            ...
            array(struct(*common_inputFields)).alias(
                "isAppealReferenceNumberAvailable_inputFields"
            ),
            array(struct(*common_inputValues)).alias(
                "isAppealReferenceNumberAvailable_inputValues"
            ),
            ...
            array(struct(*common_inputFields)).alias(
                "ccdReferenceNumberForDisplay_inputFields"
            ),
            array(struct(*common_inputValues)).alias(
                "ccdReferenceNumberForDisplay_inputValues"
            ),
...
def caseData(
    ...
    common_filter = (
        ~col("Comment").like("%Castle Park Storage%")
        & ~col("Comment").like("%Field House%")
        & ~col("Comment").like("%UT (IAC)%")
    )
    ...
    common_columns = {
        "hearingCentreDynamicList": struct(
            struct(
                col("locationCode").alias("code"), col("locationLabel").alias("label")
            ).alias("value"),
            array(
                struct(
                    lit("227101").alias("code"),
                    lit("Newport Tribunal Centre - Columbus House").alias("label"),
                ),
                struct(
                    lit("231596").alias("code"),
                    lit("Birmingham Civil And Family Justice Centre").alias("label"),
                ),
                struct(
                    lit("28837").alias("code"),
                    lit("Harmondsworth Tribunal Hearing Centre").alias("label"),
                ),
                struct(
                    lit("366559").alias("code"),
                    lit("Atlantic Quay - Glasgow").alias("label"),
                ),
                struct(
                    lit("366796").alias("code"),
                    lit("Newcastle Civil And Family Courts And Tribunals Centre").alias(
                        "label"
                    ),
                ),
                struct(
                    lit("386417").alias("code"),
                    lit("Hatton Cross Tribunal Hearing Centre").alias("label"),
                ),
                struct(
                    lit("512401").alias("code"),
                    lit(
                        "Manchester Tribunal Hearing Centre - Piccadilly Exchange"
                    ).alias("label"),
                ),
                struct(
                    lit("649000").alias("code"),
                    lit("Yarls Wood Immigration And Asylum Hearing Centre").alias(
                        "label"
                    ),
                ),
                struct(
                    lit("698118").alias("code"),
                    lit("Bradford Tribunal Hearing Centre").alias("label"),
                ),
                struct(
                    lit("765324").alias("code"),
                    lit("Taylor House Tribunal Hearing Centre").alias("label"),
                ),
            ).alias("list_items"),
        ),
        "caseManagementLocationRefData": struct(
            ...
            struct(
                ...
                array(
                    ...
                    struct(
                        lit("231596").alias("code"),
                        lit("Birmingham Civil And Family Justice Centre").alias(
                            "label"
                        ),
                    ),
                    struct(
    ...
    # Register the UDF
    map_postcode_to_hearing_centre_udf = udf(
        map_postcode_to_hearing_centre, StringType()
    )

    ##################################################################################################

    result_with_hearing_centre_df = (
        silver_m1.alias("m1")
        .join(
        ...
        .join(
            derived_history_df.alias("h"),
            (
                (col("m1.CaseNo") == col("h.CaseNo"))
                & (
                    col("m1.CentreId").isin(
                        77, 476, 101, 55, 296, 13, 79, 522, 406, 517, 37
                    )
                )
            ),
            how="left",
        ...
        .join(
            bronze_hearing_centres.alias("bhc2"),
            (col("h.der_prevFileLocation") == col("bhc2.prevFileLocation")),
            how="left",
        )
        .join(silver_m2.alias("m2"), col("m1.CaseNo") == col("m2.CaseNo"), how="left")
        .withColumn(
            "map_postcode_to_hearing_centre",
            when(
                (
                    col("h.der_prevFileLocation").isin(
                        "Arnhem House",
                    ...
                    | col("h.der_prevFileLocation").isNull()
                ),
                coalesce(
                    map_postcode_to_hearing_centre_udf(
                        coalesce(
                            col("m1.Rep_Postcode"),
                            col("m1.CaseRep_Postcode"),
                            ("m2.Appellant_Postcode"),
                        )
                    ),
                    lit("newport"),
                ),
            ).otherwise(lit("newport")),
        )
        .join(
            bronze_derive_hearing_centres.alias("bhc3"),
            col("map_postcode_to_hearing_centre") == col("bhc3.hearingCentre"),
            how="left",
        )
        .select(
            col("m1.CaseNo"),
            ...
            col("bhc2.hearingCentreDynamicList").alias(
                "dv_dhc2_hearingCentreDynamicList"
            ),
            col("bhc3.hearingCentreDynamicList").alias(
                "dv_dhc3_hearingCentreDynamicList"
            ),
            col("bhc2.caseManagementLocationRefData").alias(
                "dv_dhc2_caseManagementLocationRefData"
            ),
            col("bhc3.caseManagementLocationRefData").alias(
                "dv_dhc3_caseManagementLocationRefData"
            ),
            col("bhc2.selectedHearingCentreRefData").alias(
                "dv_dhc2_selectedHearingCentreRefData"
            ),
            col("bhc3.selectedHearingCentreRefData").alias(
                "dv_dhc3_selectedHearingCentreRefData"
            ),
            col("map_postcode_to_hearing_centre"),
            when(
                (
                    (col("bhc.Conditions").isNull())
                    | (col("bhc.Conditions") == "NO MAPPING REQUIRED")
                ),
                col("bhc.hearingCentre"),
            )
            .when(
                (
                    col("h.der_prevFileLocation").isin(
            ...
            .otherwise(col("bhc2.hearingCentre"))
            .alias("hearingCentre"),
            when(
                (
                    (col("bhc.Conditions").isNull())
            ...
            .otherwise(col("bhc2.staffLocation"))
            .alias("staffLocation"),
            when(
                (
                    (col("bhc.Conditions").isNull())
                ...
                col("bhc.caseManagementLocation"),
            )
            .when(
                (
                    col("h.der_prevFileLocation").isin(
                ...
                col("bhc3.caseManagementLocation"),
            )
            .when(
                col("h.der_prevFileLocation").isin(
                    "Castle Park Storage",
            ...
            .otherwise(col("bhc2.caseManagementLocation"))
            ...
            when(
                ...
                col("bhc.hearingCentreDynamicList"),
            )
            .when(
                (
                    col("h.der_prevFileLocation").isin(
                ...
                col("bhc3.hearingCentreDynamicList"),
            )
            .when(
                col("h.der_prevFileLocation").isin(
                    "Castle Park Storage",
            ...
            .otherwise(col("bhc2.hearingCentreDynamicList"))
            ...
            when(
                ...
                col("bhc.caseManagementLocationRefData"),
            )
            .when(
                (
                    col("h.der_prevFileLocation").isin(
                ...
                col("bhc3.caseManagementLocationRefData"),
            )
            .when(
                col("h.der_prevFileLocation").isin(
                    "Castle Park Storage",
            ...
            .otherwise(col("bhc2.caseManagementLocationRefData"))
            ...
            when(
                ...
                col("bhc.selectedHearingCentreRefData"),
            )
            .when(
                (
                    col("h.der_prevFileLocation").isin(
                ...
                col("bhc3.selectedHearingCentreRefData"),
            )
            .when(
                col("h.der_prevFileLocation").isin(
                    "Castle Park Storage",
            ...
            .otherwise(col("bhc2.selectedHearingCentreRefData"))
    ...
    silver_m3_ranked = silver_m3_filtered.withColumn(
        "row_num", row_number().over(window_spec)
    )

    # Filter the top-ranked rows where Outcome is not null
    silver_m3_filtered = silver_m3_ranked.filter(col("row_num") == 1).select(
        col("CaseNo"),
        col("Outcome"),
        col("OutOfTime"),
    ...
    df = (
        ...
        .join(result_with_hearing_centre_df.alias("hearing"), on="CaseNo", how="left")
        .withColumn(
            "appellantsRepresentation",
            when(
                (
                    (col("m1.dv_representation") == "LR")
                    & (col("lu_appealType").isNotNull())
                ),
                "No",
            )
        ...
        .withColumn(
            "submissionOutOfTime",
            when(col("OutOfTimeIssue") == True, lit("Yes")).otherwise(lit("No")),
        )
        .withColumn("adminDeclaration1", lit(["hasDeclared"]))
        .withColumn(
        ...
        .withColumn(
            ...
            date_format(col("m1.DateLodged"), "yyyy-MM-dd"),
        )
        .withColumn(
            "tribunalReceivedDate",
            date_format(col("m1.DateAppealReceived"), "yyyy-MM-dd"),
        )
        .withColumn(
            "recordedOutOfTimeDecision",
            when(
                (col("m1.dv_TargetState").isin(["paymentPending", "appealSubmitted"]))
                & (col("m1.OutOfTimeIssue") == True)
                & (col("m3.Outcome").isNotNull())
                & (col("m3.Outcome") != 0),
                lit("Yes"),
            )
            .when(
                (col("m1.dv_TargetState").isin(["paymentPending", "appealSubmitted"]))
                & (col("m1.OutOfTimeIssue") == True)
                & ((col("m3.Outcome").isNull()) | (col("m3.Outcome") == 0)),
                lit("No"),
            )
            .when(
                (~col("m1.dv_TargetState").isin(["paymentPending", "appealSubmitted"]))
        ...
        .withColumn(
            ...
            when(
                (col("OutOfTimeIssue") == True) & (col("Outcome") != 0), lit("approved")
            ).otherwise(None),
        )
        .withColumn(
            "outOfTimeDecisionMaker",
            when(
                (col("OutOfTimeIssue") == True) & (col("Outcome") != 0),
                lit("Tribunal Caseworker"),
            ).otherwise(None),
        )
        .select(
            "CaseNo",
            col("appellantsRepresentation"),
            when(conditions, col("submissionOutOfTime"))
            .otherwise(None)
            .alias("submissionOutOfTime"),
            # when(conditions, col("m3.recordedOutOfTimeDecision")).otherwise(None).alias("recordedOutOfTimeDecision"),
            when(conditions, col("applicationOutOfTimeExplanation"))
            .otherwise(None)
            .alias("applicationOutOfTimeExplanation"),
            when(conditions, col("hearing.hearingCentre"))
            .otherwise(None)
            .alias("hearingCentre"),
            when(conditions, col("hearing.staffLocation"))
            .otherwise(None)
            .alias("staffLocation"),
            when(conditions, col("hearing.caseManagementLocation"))
            .otherwise(None)
            .alias("caseManagementLocation"),
            when(conditions, col("hearing.hearingCentreDynamicList"))
            .otherwise(None)
            .alias("hearingCentreDynamicList"),
            when(conditions, col("hearing.caseManagementLocationRefData"))
            .otherwise(None)
            .alias("caseManagementLocationRefData"),
            when(conditions, col("hearing.selectedHearingCentreRefData"))
            .otherwise(None)
            .alias("selectedHearingCentreRefData"),
            col("appealWasNotSubmittedReason"),
            when(conditions, col("adminDeclaration1"))
            .otherwise(None)
            .alias("adminDeclaration1"),
            when(conditions, col("appealSubmissionDate"))
            .otherwise(None)
            .alias("appealSubmissionDate"),
            when(conditions, col("appealSubmissionInternalDate"))
            .otherwise(None)
            .alias("appealSubmissionInternalDate"),
            when(conditions, col("tribunalReceivedDate"))
            .otherwise(None)
            .alias("tribunalReceivedDate"),
            when(conditions, lit([]).cast("array<int>"))
            .otherwise(None)
            .alias("caseLinks"),
    ...
    df_audit = (
        ...
        .select(
            ...
            array(struct(*common_inputFields)).alias(
                "appellantsRepresentation_inputFields"
            ),
            array(struct(*common_inputValues)).alias(
                "appellantsRepresentation_inputValues"
            ),
            col("content.appellantsRepresentation"),
            lit("yes").alias("appellantsRepresentation_Transformation"),
            # Audit submissionOutOfTime
            array(struct(*common_inputFields, lit("OutOfTimeIssue"))).alias(
                "submissionOutOfTime_inputFields"
            ),
            array(struct(*common_inputValues, col("audit.OutOfTimeIssue"))).alias(
                "submissionOutOfTime_inputValues"
            ),
            col("content.submissionOutOfTime"),
            lit("yes").alias("submissionOutOfTime_Transformation"),
            # Audit recordedOutOfTimeDecision
            array(
                struct(*common_inputFields, lit("Outcome"), lit("OutOfTimeIssue"))
            ).alias("recordedOutOfTimeDecision_inputFields"),
            array(
                struct(
                    *common_inputValues, col("m3.Outcome"), col("audit.OutOfTimeIssue")
                )
            ).alias("recordedOutOfTimeDecision_inputValues"),
            ...
            array(struct(*common_inputFields, lit("OutOfTimeIssue"))).alias(
                "applicationOutOfTimeExplanation_inputFields"
            ),
            array(struct(*common_inputValues, col("audit.OutOfTimeIssue"))).alias(
                "applicationOutOfTimeExplanation_inputValues"
            ),
            ...
            array(
                struct(
                    *common_inputFields,
                    ...
                    lit("CentreId"),
                    lit("dv_dhc2_hearingCentre"),
                )
            ).alias("hearingCentre_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit.lu_hearingCentre"),
                    ...
                    col("hearing.dv_dhc2_hearingCentre"),
                )
            ).alias("hearingCentre_inputValues"),
            ...
            array(
                struct(
                    *common_inputFields,
                    ...
                    lit("Appellant_Postcode"),
                    lit("CentreId"),
                    lit("dv_dhc2_staffLocation"),
                    lit("dv_dhc3_staffLocation"),
                )
            ).alias("staffLocation_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit.lu_staffLocation"),
                    col("hearing.lu_conditions"),
                    col("hearing.dv_prevFileLocation"),
                    col("hearing.Rep_Postcode"),
                    col("hearing.CaseRep_Postcode"),
                    col("hearing.Appellant_Postcode"),
                    col("hearing.CentreId"),
                    col("hearing.dv_dhc2_staffLocation"),
                    lit("hearing.dv_dhc3_staffLocation"),
                )
            ).alias("staffLocation_inputValues"),
            ...
            array(
                struct(
                    *common_inputFields,
                    ...
                    lit("Appellant_Postcode"),
                    lit("CentreId"),
                    lit("dv_dhc2_caseManagementLocation"),
                    lit("dv_dhc3_caseManagementLocation"),
                )
            ).alias("caseManagementLocation_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit.lu_caseManagementLocation"),
                    col("hearing.lu_conditions"),
                    col("hearing.dv_prevFileLocation"),
                    col("hearing.Rep_Postcode"),
                    col("hearing.CaseRep_Postcode"),
                    col("hearing.Appellant_Postcode"),
                    col("hearing.CentreId"),
                    col("hearing.dv_dhc2_caseManagementLocation"),
                    col("hearing.dv_dhc3_caseManagementLocation"),
                )
            ).alias("caseManagementLocation_inputValues"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("dv_hearingCentreDynamicList"),
                    lit("lu_conditions"),
                    lit("dv_prevFileLocation"),
                    lit("Rep_Postcode"),
                    lit("CaseRep_Postcode"),
                    lit("Appellant_Postcode"),
                    lit("CentreId"),
                    lit("dv_dhc2_hearingCentreDynamicList"),
                    lit("dv_dhc3_hearingCentreDynamicList"),
                )
            ).alias("hearingCentreDynamicList_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit.dv_hearingCentreDynamicList"),
                    col("hearing.lu_conditions"),
                    col("hearing.dv_prevFileLocation"),
                    col("hearing.Rep_Postcode"),
                    col("hearing.CaseRep_Postcode"),
                    col("hearing.Appellant_Postcode"),
                    col("hearing.CentreId"),
                    col("hearing.dv_dhc2_hearingCentreDynamicList"),
                    col("hearing.dv_dhc3_hearingCentreDynamicList"),
                )
            ).alias("hearingCentreDynamicList_inputValues"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("dv_caseManagementLocationRefData"),
                    lit("lu_conditions"),
                    lit("dv_prevFileLocation"),
                    lit("Rep_Postcode"),
                    lit("CaseRep_Postcode"),
                    lit("Appellant_Postcode"),
                    lit("CentreId"),
                    lit("dv_dhc2_caseManagementLocationRefData"),
                    lit("dv_dhc3_caseManagementLocationRefData"),
                )
            ).alias("caseManagementLocationRefData_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit.dv_caseManagementLocationRefData"),
                    col("hearing.lu_conditions"),
                    col("hearing.dv_prevFileLocation"),
                    col("hearing.Rep_Postcode"),
                    col("hearing.CaseRep_Postcode"),
                    col("hearing.Appellant_Postcode"),
                    col("hearing.CentreId"),
                    col("hearing.dv_dhc2_caseManagementLocationRefData"),
                    col("hearing.dv_dhc3_caseManagementLocationRefData"),
                )
            ).alias("caseManagementLocationRefData_inputValues"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("lu_selectedHearingCentreRefData"),
                    lit("dv_prevFileLocation"),
                    lit("Rep_Postcode"),
                    lit("CaseRep_Postcode"),
                    lit("Appellant_Postcode"),
            ...
            ).alias("selectedHearingCentreRefData_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit.lu_selectedHearingCentreRefData"),
                    col("hearing.lu_conditions"),
                    col("hearing.dv_prevFileLocation"),
                    col("hearing.Rep_Postcode"),
                    col("hearing.CaseRep_Postcode"),
                    col("hearing.Appellant_Postcode"),
                    col("hearing.CentreId"),
                    col("hearing.dv_dhc2_selectedHearingCentreRefData"),
                    col("hearing.dv_dhc3_selectedHearingCentreRefData"),
                )
            ).alias("selectedHearingCentreRefData_inputValues"),
            ...
            array(struct(*common_inputFields)).alias(
                "appealWasNotSubmittedReason_inputFields"
            ),
            array(struct(*common_inputValues)).alias(
                "appealWasNotSubmittedReason_inputValues"
            ),
            ...
            array(struct(*common_inputFields, lit("DateLodged"))).alias(
                "appealSubmissionDate_inputFields"
            ),
            array(struct(*common_inputValues, col("audit.DateLodged"))).alias(
                "appealSubmissionDate_inputValues"
            ),
            col("content.appealSubmissionDate"),
            lit("yes").alias("appealSubmissionDate_Transformation"),
            # Audit appealSubmissionInternalDate
            array(struct(*common_inputFields, lit("DateLodged"))).alias(
                "appealSubmissionInternalDate_inputFields"
            ),
            array(struct(*common_inputValues, col("audit.DateLodged"))).alias(
                "appealSubmissionInternalDate_inputValues"
            ),
            ...
            array(struct(*common_inputFields, lit("DateLodged"))).alias(
                "tribunalReceivedDate_inputFields"
            ),
            array(struct(*common_inputValues, col("audit.DateLodged"))).alias(
                "tribunalReceivedDate_inputValues"
            ),
            ...
            array(
                struct(*common_inputFields, lit("OutOfTimeIssue"), lit("Outcome"))
            ).alias("outOfTimeDecisionType_inputFields"),
            array(
                struct(
                    *common_inputValues, col("audit.OutOfTimeIssue"), col("m3.Outcome")
                )
            ).alias("outOfTimeDecisionType_inputValues"),
            col("content.outOfTimeDecisionType"),
            lit("yes").alias("outOfTimeDecisionType_Transformation"),
            array(
                struct(*common_inputFields, lit("OutOfTimeIssue"), lit("Outcome"))
            ).alias("outOfTimeDecisionMaker_inputFields"),
            array(
                struct(
                    *common_inputValues, col("audit.OutOfTimeIssue"), col("m3.Outcome")
                )
            ).alias("outOfTimeDecisionMaker_inputValues"),
...
def flagsLabels(silver_m1, silver_m2, silver_c):
    ...
    case_flag_lookup = {
        ...
        8: {
            "name": "Other",
            "code": "OT0001",
            "comment": "Reclassified RFT",
            "hearing": "Yes",
        },
        24: {
            "name": "Other",
            "code": "OT0001",
            "comment": "EEA Family Permit",
            "hearing": "Yes",
        },
        25: {
    ...
    def make_flag_struct(name, code, comment, hearing):
        return struct(
            ...
            struct(
                lit(name).alias("name"),
                array(
                    struct(expr("uuid()").alias("id"), lit("Case").alias("value"))
                ).alias("path"),
    ...
    def generate_case_flag_details(col_category_ids, col_hoanref):
        ...
        ot0001_comment = (
            when(col_hoanref.isNotNull(), "Dropped Case")
    ...
    def generate_appellant_flag_details(col_category_ids, col_detained):
        ...
        pf0012_data = appellant_flag_lookup[
            pf0012_cat_ids[0]
    ...
    grouped = grouped.withColumn(
        "caseFlagDetails",
        generate_case_flag_details(col("CategoryIds"), col("HOANRef")),
    )
    grouped = grouped.withColumn(
        "appellantFlagDetails",
        generate_appellant_flag_details(col("CategoryIds"), col("Detained")),
    )
    ## Formatting caseFlags into final structure or Null/None if no conditions are met
    grouped = grouped.withColumn(
        "caseFlags",
        when(
            size(col("caseFlagDetails")) > 0,
            struct(
    ...
    grouped = grouped.withColumn(
        "appellantLevelFlags",
        when(
            F.size("appellantFlagDetails") > 0,
            struct(
                col("appellantFlagDetails").alias("details"),
                # lit(None).cast("string").alias("groupId"),
                concat_ws(" ", col("AppellantForenames"), col("AppellantName")).alias(
                    "partyName"
                ),
    ...
    grouped = grouped.withColumn(
        ...
        when(col("CasePrefix") == "DA", lit("Yes")).otherwise(lit("No")),
    )
    ...
    df = grouped.select(
        ...
        when(conditions, col("appellantLevelFlags"))
        .otherwise(None)
        .alias("appellantLevelFlags"),
        when(conditions, col("s94bStatus")).otherwise(None).alias("s94bStatus"),
        # col("journeyType"),
        when(conditions, col("isAdmin")).otherwise(None).alias("isAdmin"),
        when(conditions, col("isAriaMigratedFeeExemption"))
        .otherwise(None)
        .alias("isAriaMigratedFeeExemption"),
    ...
    df_audit = (
        ...
        .join(grouped.alias("grp"), ["CaseNo"], "left")
        .select(
            col("CaseNo"),
            # Audit caseFlags
            array(
                struct(*common_inputFields, lit("CategoryIds"), lit("HOANRef"))
            ).alias("caseFlags_inputFields"),
            array(
                struct(
                    *common_inputValues, col("grp.CategoryIds"), col("audit.HOANRef")
                )
            ).alias("caseFlags_inputValues"),
            col("content.caseFlags"),
            lit("yes").alias("caseFlags_Transformation"),
            # Audit appellantLevelFlags
            array(
                struct(*common_inputFields, lit("CategoryIds"), lit("Detained"))
            ).alias("appellantLevelFlags_inputFields"),
            array(
                struct(*common_inputValues, col("grp.CategoryIds"), col("m2.Detained"))
            ).alias("appellantLevelFlags_inputValues"),
            ...
            array(struct(*common_inputFields, lit("CasePrefix"))).alias(
                "isAriaMigratedFeeExemption_inputFields"
            ),
            array(struct(*common_inputValues, col("audit.CasePrefix"))).alias(
                "isAriaMigratedFeeExemption_inputValues"
            ),
...
def cleanPhoneNumber(PhoneNumber):
    ...
    PhoneNumber = re.sub(
        r"[.\-]", "", PhoneNumber
    )  # 17. Remove internal dots and dashes
    PhoneNumber = re.sub(
        r"^0044", "+44", PhoneNumber
    )  # 18. Change starting 0044 to +44
    PhoneNumber = re.sub(
        r"\(0\)", "", PhoneNumber
    )  # 19. Remove (0) if +44 or 44 exists in the number
    PhoneNumber = re.sub(
        r"^44", "+44", PhoneNumber
    )  # 20. Add missing + if number starts with 44 (but not already +)
...
def legalRepDetails(silver_m1, bronze_countryFromAddress):
    conditions_legalRepDetails = (col("dv_representation") == "LR") & (
        col("lu_appealType").isNotNull()
    )

    staging_df = (
        silver_m1.alias("m1")
        .filter(conditions_legalRepDetails)
        .withColumn(
            "legalRepGivenName",
            coalesce(
                col("Contact"), col("Rep_Name"), col("CaseRep_Name")
            ),  # If contact is null use RepName, if RepName null use CaseRepName
        )
        .withColumn(
            "legalRepFamilyNamePaperJ",
            coalesce(
                col("Rep_Name"), col("CaseRep_Name")
            ),  # If RepName is null use CaseRepName
        )
        .withColumn(
        ...
        .withColumn(
            ...
            cleanEmailUDF(
                coalesce(
                    col("Rep_Email"),
                    col("CaseRep_Email"),
                    col("CaseRep_FileSpecific_Email"),
                )
            ),
        )
        .withColumn(
        ...
        .withColumn(
            ...
            .when(
                (col("RepresentativeId") == 0) & (col("ukPostcode") == "True"), "Yes"
            )  # 0 + UK postcode
            .when(
                (col("RepresentativeId") == 0)
                & (col("countryFromAddress").contains("United Kingdom")),
                "Yes",
            )  # 0 + UK country
        ...
        .withColumn(
            "oocAddressLine1",
            when(
                col("legalRepHasAddress") == lit("Yes"), None
            ).otherwise(  # No address if legalRepHasAddress is No
                coalesce(
                    col("CaseRep_Address1"),
                    col("CaseRep_Address2"),
                    col("CaseRep_Address3"),
        ...
        .withColumn(
            "oocAddressLine2",
            when(col("legalRepHasAddress") == lit("Yes"), None).otherwise(
                coalesce(
                    col("CaseRep_Address2"),
                    col("CaseRep_Address3"),
                    col("CaseRep_Address4"),
        ...
        .withColumn(
            ...
            when(col("legalRepHasAddress") == lit("Yes"), None).otherwise(
                getCountryLRUDF(col("legalRepAddressUK"))
            ),  # Use UDF getCountryLR(). Sample data indicates function works.
        )
        .withColumn(
            "legalRepAddressUK",
            ...
            .when(
                ...
                struct(
                    ...
                    when(
                        ...
                        & col("CaseRep_Address5").isNull(),
                        coalesce(
                            col("CaseRep_Address2"), col("CaseRep_Postcode"), lit("")
                        ),
                    )
                    .otherwise(
                        coalesce(
                    ...
                    .alias("PostTown"),
                    # County
                    when(col("CaseRep_Address5").isNull(), lit(""))
                    ...
                    .otherwise(coalesce(col("CaseRep_Address5"), lit("")))
                    .alias("Country"),
                    # Postcode
            ...
            .otherwise(
                struct(
                    ...
                    .alias("PostTown"),
                    # County
                    when(col("Rep_Address5").isNull(), lit(""))
                    .otherwise(col("Rep_Address4"))
                    .alias("County"),
                    # Country
                    when(
                        col("Rep_Address5").isNull(),
                    ...
                    .otherwise(coalesce(col("Rep_Address5"), lit("")))
                    .alias("Country"),
                    # Postcode
    ...
    df = (
        staging_df.alias("d1")
        .join(
            bronze_countryFromAddress.alias("b1"),
            col("d1.oocLrCountryGovUkAdminJ") == col("b1.countryFromAddress"),
            "left",
        )
        .drop(
            col("d1.oocLrCountryGovUkAdminJ"),
            col("b1.countryGovUkOocAdminJ"),
            col("b1.lu_countryGovUkOocAdminJ"),
            col("b1.countryFromAddress"),
        )
    ...
    df_audit = (
        ...
        .select(
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("m1_audit.Contact"),
                    lit("m1_audit.Rep_Name"),
                    lit("m1_audit.CaseRep_Name"),
                )
            ).alias("legalRepGivenName_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("m1_audit.Contact"),
                    col("m1_audit.Rep_Name"),
                    col("m1_audit.CaseRep_Name"),
                )
            ).alias("legalRepGivenName_inputValues"),
            ...
            ).alias("legalRepFamilyNamePaperJ_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("m1_audit.Rep_Name"),
            ...
            ).alias("legalRepFamilyNamePaperJ_inputValues"),
            ...
            ).alias("legalRepCompanyPaperJ_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("m1_audit.Rep_Name"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("m1_audit.Rep_Email"),
                    lit("m1_audit.CaseRep_Email"),
                    lit("m1_audit.CaseRep_FileSpecific_Email"),
                )
            ).alias("legalRepEmail_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("m1_audit.Rep_Email"),
                    col("m1_audit.CaseRep_Email"),
                    col("m1_audit.CaseRep_FileSpecific_Email"),
                )
            ).alias("legalRepEmail_inputValues"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("m1_audit.Rep_Address1"),
                    lit("m1_audit.Rep_Address2"),
                    lit("m1_audit.Rep_Address3"),
                    lit("m1_audit.Rep_Address4"),
                    lit("m1_audit.Rep_Address5"),
                    lit("m1_audit.Rep_Postcode"),
                    lit("m1_audit.CaseRep_Address1"),
                    lit("m1_audit.CaseRep_Address2"),
                    lit("m1_audit.CaseRep_Address3"),
                    lit("m1_audit.CaseRep_Address4"),
                    lit("m1_audit.CaseRep_Address5"),
                    lit("m1_audit.CaseRep_Postcode"),
                )
            ).alias("legalRepAddressUK_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("m1_audit.Rep_Address1"),
                    col("m1_audit.Rep_Address2"),
                    col("m1_audit.Rep_Address3"),
                    col("m1_audit.Rep_Address4"),
                    col("m1_audit.Rep_Address5"),
                    col("m1_audit.Rep_Postcode"),
                    col("m1_audit.CaseRep_Address1"),
                    col("m1_audit.CaseRep_Address2"),
                    col("m1_audit.CaseRep_Address3"),
                    col("m1_audit.CaseRep_Address4"),
                    col("m1_audit.CaseRep_Address5"),
                    col("m1_audit.CaseRep_Postcode"),
                )
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("m1_audit.CaseRep_Postcode"),
                    lit("content.legalRepAddressUK"),
                    lit("m1_audit.RepresentativeId"),
                )
            ).alias("legalRepHasAddress_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("m1_audit.CaseRep_Postcode"),
                    col("content.legalRepAddressUK"),
                    col("m1_audit.RepresentativeId"),
                )
            ).alias("legalRepHasAddress_inputValues"),
            ...
            ).alias("oocAddressLine1_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("m1_audit.CaseRep_Address1"),
            ...
            ).alias("oocAddressLine1_inputValues"),
            ...
            ).alias("oocAddressLine2_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("m1_audit.CaseRep_Address2"),
            ...
            ).alias("oocAddressLine2_inputValues"),
            ...
            ).alias("oocAddressLine3_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("m1_audit.CaseRep_Address3"),
            ...
            ).alias("oocAddressLine3_inputValues"),
            ...
            ).alias("oocAddressLine4_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("m1_audit.CaseRep_Address4"),
            ...
            ).alias("oocAddressLine4_inputValues"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("content.oocAddressLine1"),
                    lit("content.oocAddressLine2"),
                    lit("content.oocAddressLine3"),
                    lit("content.oocAddressLine4"),
                    lit("m1_audit.CaseRep_Postcode"),
                )
            ).alias("oocLrCountryGovUkAdminJ_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("content.oocAddressLine1"),
            ...
            ).alias("oocLrCountryGovUkAdminJ_inputValues"),
...
def makeFullAddress(a1, a2, a3, a4, a5, postcode):
...
def searchCountry(ukPostcodeAppellant, appellantFullAddress):
    full_address = (
        ", ".join(filter(None, appellantFullAddress)) if appellantFullAddress else ""
    )

    if (
        ukPostcodeAppellant is True
        or ukPostcodeAppellant == "True"
        or any(
            line is not None and line.lower() in ["gb", "uk", "united kingdom"]
            for line in appellantFullAddress
        )
    ):
        return "GB"
    else:
...
def getCountryApp(countryId, country, ukPostcodeAppellant, appellantFullAddress):
    countryFromAddress = []
    try:
        if (country is not None and countryId != 0) or (
            country is not None and pd.notna(country)
        ):
            ...
            countryFromAddress.append(
                searchCountry(ukPostcodeAppellant, appellantFullAddress)
            )
            return ",".join(countryFromAddress)
        else:
...
def getIsInUk(countryId, ukPostcodeAppellant, appellantFullAddress):
    try:
        if (
            countryId == 188
        ):  # 188 = UK Country ID, 'GB' is standard country code for UK.
            return True
        else:
...
def appellantDetails(
    ...
    appellant_nationalities = (
        silver_m1.join(bronze_nationalities, on="NationalityId", how="left")
        .withColumn(
            "appellantStateless",
        ...
        .select(
            ...
            "NationalityId",
            "Description",
            when(col("countryCode") == lit("NO MAPPING REQUIRED"), lit(None))
            .otherwise(col("countryCode"))
            .alias("countryCode"),
            when(
                col("appellantNationalitiesDescription") == lit("NO MAPPING REQUIRED"),
                lit(None),
            )
            .otherwise(col("appellantNationalitiesDescription"))
            .alias("appellantNationalitiesDescription"),
            "appellantStateless",
        )
    )
    ...
    is_minor_expr = (
        when(
            conditions
            & ((datediff(col("DateLodged"), col("BirthDate")) / 365.25) < 18),
            lit("Yes"),
        )
        .when(conditions, lit("No"))
        .otherwise(None)
    )
    ...
    deportation_condition = (
        (
            col("CategoryIdList").isNotNull()
            & (
                expr(
                    f"size(array_intersect(CategoryIdList, array({','.join(map(str, deportation_category_ids))})) )"
                )
                > 0
            )
        )
    ...
    # appellantInUk logic
    appellant_in_uk_expr = (
        when(conditions & (expr("array_contains(CategoryIdList, 37)")), lit("Yes"))
        .when(conditions & (expr("array_contains(CategoryIdList, 38)")), lit("No"))
        .when(conditions & (col("dv_addressInUk")), lit("Yes"))
        .when(conditions & (~col("dv_addressInUk")), lit("No"))
        .otherwise(None)
    )

    # appealOutOfCountry logic
    appeal_out_of_country_expr = (
        when(appellant_in_uk_expr == lit("Yes"), lit("No"))
        .when(appellant_in_uk_expr == lit("No"), lit("Yes"))
        .otherwise(None)
    )
    ...
    appellant_has_fixed_address_adminj_expr = when(
    ...
    address_line2_adminj_expr = when(
        ...
        coalesce(
            ...
            when(col("Appellant_Address1").isNull(), None).otherwise(
                coalesce(
                    when(
                        col("Appellant_Address2") != col("Appellant_Address1"),
                        col("Appellant_Address2"),
                    ),
                    when(
                        col("Appellant_Address3") != col("Appellant_Address1"),
                        col("Appellant_Address3"),
                    ),
                    when(
                        col("Appellant_Address4") != col("Appellant_Address1"),
                        col("Appellant_Address4"),
                    ),
                    when(
                        col("Appellant_Address5") != col("Appellant_Address1"),
                        col("Appellant_Address5"),
                    ),
                    when(
                        col("Appellant_Postcode") != col("Appellant_Address1"),
                        col("Appellant_Postcode"),
                    ),
                )
            ),
            when(
                col("Appellant_Address2").isNull() & col("Appellant_Address1").isNull(),
                None,
            ).otherwise(
                coalesce(
                    when(
                        col("Appellant_Address3") != col("Appellant_Address2"),
                        col("Appellant_Address3"),
                    ),
                    when(
                        col("Appellant_Address4") != col("Appellant_Address2"),
                        col("Appellant_Address4"),
                    ),
                    when(
                        col("Appellant_Address5") != col("Appellant_Address2"),
                        col("Appellant_Address5"),
                    ),
                    when(
                        col("Appellant_Postcode") != col("Appellant_Address2"),
                        col("Appellant_Postcode"),
                    ),
                )
            ),
            when(
                col("Appellant_Address3").isNull()
                & col("Appellant_Address2").isNull()
                & col("Appellant_Address1").isNull(),
                None,
            ).otherwise(
                coalesce(
                    when(
                        col("Appellant_Address4") != col("Appellant_Address3"),
                        col("Appellant_Address4"),
                    ),
                    when(
                        col("Appellant_Address5") != col("Appellant_Address3"),
                        col("Appellant_Address5"),
                    ),
                    when(
                        col("Appellant_Postcode") != col("Appellant_Address3"),
                        col("Appellant_Postcode"),
                    ),
                )
            ),
            when(
                col("Appellant_Address4").isNull()
                & col("Appellant_Address3").isNull()
                & col("Appellant_Address2").isNull()
                & col("Appellant_Address1").isNull(),
                None,
                ...
                coalesce(
                    when(
                        col("Appellant_Address5") != col("Appellant_Address4"),
                        col("Appellant_Address5"),
                    ),
                    when(
                        col("Appellant_Postcode") != col("Appellant_Address4"),
                        col("Appellant_Postcode"),
                    ),
                )
            ),
            when(
                col("Appellant_Address5").isNull()
                & col("Appellant_Address4").isNull()
                & col("Appellant_Address3").isNull()
                & col("Appellant_Address2").isNull()
                & col("Appellant_Address1").isNull(),
                ...
                coalesce(
                    when(
                        col("Appellant_Postcode") != col("Appellant_Address5"),
                        col("Appellant_Postcode"),
                    )
                )
            ),
    ...
    address_line3_adminj_expr = when(
        ...
        & (
            col("Appellant_Address3").isNotNull()
            | col("Appellant_Address4").isNotNull()
        ),
    ...
    address_line4_adminj_expr = when(
        ...
        & expr("array_contains(CategoryIdList, 38)")
        & (
            col("Appellant_Address5").isNotNull()
            | col("Appellant_Postcode").isNotNull()
        ),
    ...
    bronze_countries_countryFromAddress = bronze_countryFromAddress.withColumn(
        "lu_cfa_countryGovUkOocAdminJ", col("countryGovUkOocAdminJ")
    ).withColumn("lu_cfa_countryFromAddress", col("countryFromAddress"))

    silver_m2_derived = (
        silver_m2_derived.alias("main")
        .join(
            bronze_countries_countryFromAddress.alias("cfa"),
            col("main.dv_countryGovUkOocAdminJ")
            == col("cfa.lu_cfa_countryFromAddress"),
            "left",
        )
        .select(
            "main.*",
            when(
                col("lu_cfa_countryFromAddress").isNotNull(),
                col("lu_cfa_countryGovUkOocAdminJ"),
            )
            .otherwise(col("dv_countryGovUkOocAdminJ"))
            .alias("countryGovUkOocAdminJ"),
        )
    )
    ...
    country_gov_uk_ooc_adminj_expr = when(
        conditions & expr("array_contains(CategoryIdList, 38)"),
        when(
            col("countryGovUkOocAdminJ").isin(["NO MAPPING REQUIRED"]), lit(None)
        ).otherwise(col("countryGovUkOocAdminJ")),
    ).otherwise(None)

    # internalAppellantEmail logic
    internal_appellant_email_expr = when(
        conditions & col("Appellant_Email").isNotNull(),
        cleanEmailUDF(col("Appellant_Email")),
    ).otherwise(None)

    # internalAppellantMobileNumber logic
    internal_appellant_mobile_number_expr = when(
        conditions & col("Appellant_Telephone").isNotNull(),
        filterMobilePhoneNumberUDF(col("Appellant_Telephone")),
    ).otherwise(None)
    ...
    df = (
        ...
        .select(
            col("CaseNo"),
            when(conditions, col("Appellant_Name"))
            .otherwise(None)
            .alias("appellantFamilyName"),
            when(conditions, col("Appellant_Forenames"))
            .otherwise(None)
            .alias("appellantGivenNames"),
            when(
                conditions,
                concat(col("Appellant_Forenames"), lit(" "), col("Appellant_Name")),
            )
            .otherwise(None)
            .alias("appellantNameForDisplay"),
            when(conditions, date_format(col("BirthDate"), "yyyy-MM-dd"))
            .otherwise(None)
            .alias("appellantDateOfBirth"),
            ...
            .otherwise(None)
            .alias("caseNameHmctsInternal"),
            when(
                conditions,
                concat(col("Appellant_Forenames"), lit(" "), col("Appellant_Name")),
            )
            .otherwise(None)
            .alias("hmctsCaseNameInternal"),
            internal_appellant_email_expr.alias("internalAppellantEmail"),
            email_expr.alias("email"),
            internal_appellant_mobile_number_expr.alias(
            ...
            appellant_has_fixed_address_adminj_expr.alias(
            ...
            col("appellantNationalitiesDescription").alias(
                "appellantNationalitiesDescription"
            ),
            when(conditions & deportation_condition, lit("Yes"))
            .when(conditions, lit("No"))
            .otherwise(None)
            .alias("deportationOrderOptions"),
        )
        .distinct()
    )
    ...
    df_audit = (
        silver_m1.join(silver_m2_derived, ["CaseNo"], "left")
        .alias("audit")
        .join(silver_c_grouped, ["CaseNo"], "left")
        .join(df.alias("content"), ["CaseNo"], "left")
        ...
        .select(
            ...
            array(struct(*common_inputFields, lit("Appellant_Name"))).alias(
                "appellantFamilyName_inputFields"
            ),
            array(struct(*common_inputValues, col("Appellant_Name"))).alias(
                "appellantFamilyName_inputValues"
            ),
            col("content.appellantFamilyName"),
            lit("no").alias("appellantFamilyName_Transformation"),
            # Audit appellantGivenNames
            array(struct(*common_inputFields, lit("Appellant_Forenames"))).alias(
                "appellantGivenNames_inputfields"
            ),
            array(struct(*common_inputValues, col("Appellant_Forenames"))).alias(
                "appellantGivenNames_inputvalues"
            ),
            ...
            ).alias("appellantNameForDisplay_inputfields"),
            array(
                struct(
                    *common_inputValues,
                    col("Appellant_Name"),
            ...
            ).alias("appellantNameForDisplay_inputvalues"),
            col("content.appellantNameForDisplay"),
            lit("no").alias("appellantNameForDisplay_Transformation"),
            # Audit appellantDateOfBirth
            array(struct(*common_inputFields, lit("BirthDate"))).alias(
                "appellantDateOfBirth_inputfields"
            ),
            array(struct(*common_inputValues, col("BirthDate"))).alias(
                "appellantDateOfBirth_inputvalues"
            ),
            ...
            ).alias("caseNameHmctsInternal_inputfields"),
            array(
                struct(
                    *common_inputValues,
                    col("Appellant_Name"),
            ...
            ).alias("caseNameHmctsInternal_inputvalues"),
            ...
            ).alias("hmctsCaseNameInternal_inputfields"),
            array(
                struct(
                    *common_inputValues,
                    col("Appellant_Name"),
            ...
            ).alias("hmctsCaseNameInternal_inputvalues"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("CategoryId"),
                    lit("dv_CCDAppealType"),
                    lit("DeportationDate"),
                    lit("RemovalDate"),
                )
            ).alias("deportationOrderOptions_inputfields"),
            array(
                struct(
                    *common_inputValues,
                    col("CategoryIdList"),
            ...
            ).alias("deportationOrderOptions_inputvalues"),
            col("content.deportationOrderOptions"),
            lit("yes").alias("deportationOrderOptions_Transformation"),
            # Audit appellantInUk
            array(
                struct(*common_inputFields, lit("CategoryId"), lit("dv_addressInUk"))
            ).alias("appellantInUk_inputfields"),
            array(
                struct(
                    *common_inputValues,
                    col("CategoryIdList"),
                    col("audit.dv_addressInUk"),
                )
            ).alias("appellantInUk_inputvalues"),
            col("content.appellantInUk"),
            lit("yes").alias("appellantInUk_Transformation"),
            # Audit appealOutOfCountry
            array(struct(*common_inputFields, lit("appellantInUk"))).alias(
                "appealOutOfCountry_inputfields"
            ),
            array(struct(*common_inputValues, col("content.appellantInUk"))).alias(
                "appealOutOfCountry_inputvalues"
            ),
            col("content.appealOutOfCountry"),
            lit("yes").alias("appealOutOfCountry_Transformation"),
            # Audit isAppellantMinor
            array(
                struct(*common_inputFields, lit("BirthDate"), lit("DateLodged"))
            ).alias("isAppellantMinor_inputfields"),
            array(
                struct(*common_inputValues, col("BirthDate"), col("DateLodged"))
            ).alias("isAppellantMinor_inputvalues"),
            col("content.isAppellantMinor"),
            lit("yes").alias("isAppellantMinor_Transformation"),
            # Audit appellantStateless
            array(struct(*common_inputFields, lit("NationalityId"))).alias(
                "appellantStateless_inputfields"
            ),
            array(struct(*common_inputValues, col("NationalityId"))).alias(
                "appellantStateless_inputvalues"
            ),
            col("content.appellantStateless"),
            lit("yes").alias("appellantStateless_Transformation"),
            # Audit appellantNationalities
            array(struct(*common_inputFields, lit("appellantNationalities"))).alias(
                "appellantNationalities_inputfields"
            ),
            array(struct(*common_inputValues, col("appellantNationalities"))).alias(
                "appellantNationalities_inputvalues"
            ),
            ...
            array(
                struct(*common_inputFields, lit("lu_appellantNationalitiesDescription"))
            ).alias("lu_appellantNationalitiesDescription_inputfields"),
            array(
                struct(*common_inputValues, col("appellantNationalitiesDescription"))
            ).alias("appellantNationalitiesDescription_inputvalues"),
            ...
            array(struct(*common_inputFields, lit("CategoryIdList"))).alias(
                "appellantHasFixedAddress_inputfields"
            ),
            array(struct(*common_inputValues, col("CategoryIdList"))).alias(
                "appellantHasFixedAddress_inputvalues"
            ),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("Appellant_Address1"),
                    lit("Appellant_Address2"),
                    lit("Appellant_Address3"),
                    lit("Appellant_Address4"),
                    lit("Appellant_Address5"),
                    lit("Appellant_Postcode"),
                    lit("CategoryIdList"),
                )
            ).alias("appellantAddress_inputfields"),
            array(
                struct(
                    *common_inputValues,
                    col("Appellant_Address1"),
            ...
            ).alias("appellantAddress_inputvalues"),
            ...
            ).alias("oocAppealAdminJ_inputfields"),
            array(
                struct(
                    *common_inputValues,
                    col("CategoryIdList"),
                    col("HORef"),
                    col("FCONumber"),
                    col("content_c.lu_HORef"),
                )
            ).alias("oocAppealAdminJ_inputvalues"),
            col("content.oocAppealAdminJ"),
            lit("yes").alias("oocAppealAdminJ_Transformation"),
            # Audit appellantHasFixedAddressAdminJ
            array(struct(*common_inputFields, lit("CategoryIdList"))).alias(
                "appellantHasFixedAddressAdminJ_inputfields"
            ),
            array(struct(*common_inputValues, col("CategoryIdList"))).alias(
                "appellantHasFixedAddressAdminJ_inputvalues"
            ),
            ...
            ).alias("addressLine1AdminJ_inputfields"),
            array(
                struct(
                    *common_inputValues,
                    col("Appellant_Address1"),
            ...
            ).alias("addressLine1AdminJ_inputvalues"),
            ...
            ).alias("addressLine2AdminJ_inputfields"),
            array(
                struct(
                    *common_inputValues,
                    col("Appellant_Address2"),
            ...
            ).alias("addressLine2AdminJ_inputvalues"),
            ...
            ).alias("addressLine3AdminJ_inputfields"),
            array(
                struct(
                    *common_inputValues,
                    col("Appellant_Address3"),
            ...
            ).alias("addressLine3AdminJ_inputvalues"),
            ...
            ).alias("addressLine4AdminJ_inputfields"),
            array(
                struct(
                    *common_inputValues,
                    col("Appellant_Address5"),
            ...
            ).alias("addressLine4AdminJ_inputvalues"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("AppellantCountryId"),
                    lit("ukPostcodeAppellant"),
                    lit("appellantFullAddress"),
                    lit("CategoryIdList"),
                )
            ).alias("countryGovUkOocAdminJ_inputfields"),
            array(
                struct(
                    *common_inputValues,
                    col("AppellantCountryId"),
            ...
            ).alias("countryGovUkOocAdminJ_inputvalues"),
            col("content.countryGovUkOocAdminJ"),
            lit("yes").alias("countryGovUkOocAdminJ_Transformation"),
            # Audit internalAppellantEmail
            array(struct(*common_inputFields, lit("Appellant_Email"))).alias(
                "internalAppellantEmail_inputfields"
            ),
            array(struct(*common_inputValues, col("Appellant_Email"))).alias(
                "internalAppellantEmail_inputvalues"
            ),
            col("content.internalAppellantEmail"),
            lit("yes").alias("internalAppellantEmail_Transformation"),
            # Audit internalAppellantMobileNumber
            array(struct(*common_inputFields, lit("Appellant_Telephone"))).alias(
                "internalAppellantMobileNumber_inputfields"
            ),
            array(struct(*common_inputValues, col("Appellant_Telephone"))).alias(
                "internalAppellantMobileNumber_inputvalues"
            ),
            ...
            array(struct(*common_inputFields, lit("Appellant_Telephone"))).alias(
                "mobileNumber_inputfields"
            ),
            array(struct(*common_inputValues, col("Appellant_Telephone"))).alias(
                "mobileNumber_inputvalues"
            ),
...
def homeOfficeDetails(silver_m1, silver_m2, silver_c, bronze_HORef_cleansing):
    ...
    decision_letter_received_date_expr = when(
        ...
        & (
            ~coalesce(col("lu_HORef"), col("HORef"), col("FCONumber"), lit("")).like(
                "%GWF%"
            )
        )
    ...
    df = (
        ...
        .select(
            ...
            when(
                home_office_reference_number_expr.isNull()
                & gwf_reference_number_expr.isNull(),
                "999999999",
            )
            .otherwise(home_office_reference_number_expr)
            .alias("homeOfficeReferenceNumber"),
    ...
    df_audit = (
        ...
        .select(
            col("CaseNo"),
            array(
                struct(*common_inputFields, lit("audit_m2.dv_appellantIsInUk"))
            ).alias("homeOfficeDecisionDate_inputFields"),
            array(
                struct(*common_inputValues, col("audit_m2.dv_appellantIsInUk"))
            ).alias("homeOfficeDecisionDate_inputValues"),
            col("content.homeOfficeDecisionDate"),
            lit("yes").alias("homeOfficeDecisionDate_Transformed"),
            array(
                struct(
                    *common_inputFields,
                    lit("audit_m2.dv_appellantIsInUk"),
                    lit("content_c.lu_HORef"),
                    lit("audit.HORef"),
                    lit("audit_m2.FCONumber"),
                )
            ).alias("decisionLetterReceivedDate_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit_m2.dv_appellantIsInUk"),
                    col("content_c.lu_HORef"),
                    col("audit.HORef"),
                    col("audit_m2.FCONumber"),
                )
            ).alias("decisionLetterReceivedDate_inputValues"),
            ...
            ).alias("dateEntryClearanceDecision_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit_m2.dv_appellantIsInUk"),
            ...
            ).alias("dateEntryClearanceDecision_inputValues"),
            ...
            ).alias("homeOfficeReferenceNumber_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit_m2.dv_appellantIsInUk"),
            ...
            ).alias("homeOfficeReferenceNumber_inputValues"),
            ...
            ).alias("gwfReferenceNumber_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit_m2.dv_appellantIsInUk"),
            ...
            ).alias("gwfReferenceNumber_inputValues"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("isHomeOfficeIntegrationEnabled"),
                    lit("dv_CCDAppealType"),
                )
            ).alias("isHomeOfficeIntegrationEnabled_inputFields"),
            array(
                struct(*common_inputValues, lit("null"), col("dv_CCDAppealType"))
            ).alias("isHomeOfficeIntegrationEnabled_inputValues"),
            ...
            array(
                struct(*common_inputFields, lit("homeOfficeNotificationsEligible"))
            ).alias("homeOfficeNotificationsEligible_inputFields"),
            array(struct(*common_inputValues, lit("null"))).alias(
                "homeOfficeNotificationsEligible_inputValues"
            ),
...
def paymentType(silver_m1):
    ...
    payment_content = silver_m1.select(
        ...
        .alias("feeAmountGbp"),
        when(
            conditions_all & (col("VisitVisaType") == 1),
            "Notice of Appeal - appellant consents without hearing A",
        )
        .when(
            conditions_all & (col("VisitVisaType") == 2),
    ...
    payment_audit = silver_m1.join(payment_content, on="CaseNo").select(
        ...
        # feeWithoutHearing
        array(struct(*payment_common_inputFields)).alias(
            "feeWithoutHearing_inputFields"
        ),
        array(struct(*payment_common_inputValues)).alias(
            "feeWithoutHearing_inputValues"
        ),
        col("feeWithoutHearing"),
        lit("yes").alias("feeWithoutHearing_Transformation"),
        # paymentDescription
        array(struct(*payment_common_inputFields)).alias(
            "paymentDescription_inputFields"
        ),
        array(struct(*payment_common_inputValues)).alias(
            "paymentDescription_inputValues"
        ),
        col("paymentDescription"),
        lit("yes").alias("paymentDescription_Transformation"),
        # feePaymentAppealType (literal, so empty arrays)
        array(struct(lit("dv_CCDAppealType"))).alias(
            "feePaymentAppealType_inputFields"
        ),
        array(struct(col("dv_CCDAppealType"))).alias(
            "feePaymentAppealType_inputValues"
        ),
        ...
        array(struct(*payment_common_inputFields)).alias(
            "decisionHearingFeeOption_inputFields"
        ),
        array(struct(*payment_common_inputValues)).alias(
            "decisionHearingFeeOption_inputValues"
        ),
        ...
        array(struct(lit("dv_CCDAppealType"))).alias(
            "hasServiceRequestAlready_inputFields"
        ),
        array(struct(col("dv_CCDAppealType"))).alias(
            "hasServiceRequestAlready_inputValues"
        ),
...
def partyID(silver_m1, silver_m3, silver_c):
    ...
    df = (
        silver_m1.alias("m1")
        .join(silver_m3_filtered.alias("m3"), on="CaseNo", how="left")
        .join(silver_c_grouped.alias("c"), ["CaseNo"], "left")
        .withColumn(
            "appellantsRepresentation",
            when(
                (
        ...
        .withColumn(
            "appellantPartyId",
            when(conditions_all, expr("uuid()")).otherwise(
                expr("uuid()")
            ),  # use expr() to ensure a unique UUID per row
        )
        .withColumn(
            "legalRepIndividualPartyId",
            when((col("appellantsRepresentation") == "No"), expr("uuid()")).otherwise(
                None
            ),  # If appelRep = LR (no) then valid
        )
        .withColumn(
        ...
        .withColumn(
            "sponsorPartyId",
            when(
                conditions_all & col("m1.Sponsor_Name").isNotNull(), expr("uuid()")
            ).otherwise(None),
        )
        .select(
            col("m1.CaseNo"),
    ...
    df_audit = (
        silver_m1.join(silver_c, ["CaseNo"], "left")
        .alias("audit")
        .join(df.alias("content"), ["CaseNo"], "left")
        .select(
            col("CaseNo"),
            # Audit appellantPartyId - ARIADM-779
            array(struct(*common_inputFields, lit("content.appellantPartyId"))).alias(
                "appellantPartyId_inputFields"
            ),
            array(struct(*common_inputValues, col("content.appellantPartyId"))).alias(
                "appellantPartyId_inputValues"
            ),
            col("content.appellantPartyId"),
            lit("yes").alias("appellantPartyId_Transformation"),
            # Audit legalRepIndividualPartyId - ARIADM-779
            array(
                struct(*common_inputFields, lit("content.legalRepIndividualPartyId"))
            ).alias("legalRepIndividualPartyId_inputFields"),
            array(
                struct(*common_inputValues, col("content.legalRepIndividualPartyId"))
            ).alias("legalRepIndividualPartyId_inputValues"),
            ...
            array(
                struct(*common_inputFields, lit("content.legalRepOrganisationPartyId"))
            ).alias("legalRepOrganisationPartyId_inputFields"),
            array(
                struct(*common_inputValues, col("content.legalRepOrganisationPartyId"))
            ).alias("legalRepOrganisationPartyId_inputValues"),
            ...
            array(struct(*common_inputFields, lit("content.sponsorPartyId"))).alias(
                "sponsorPartyId_inputFields"
            ),
            array(struct(*common_inputValues, col("content.sponsorPartyId"))).alias(
                "sponsorPartyId_inputValues"
            ),
...
def remissionTypes(silver_m1, bronze_remission_lookup_df, silver_m4):
    ...
    df = (
        silver_m1.alias("m1")
        .filter(conditions_remissionTypes & conditions)
        .join(
            bronze_remission_lookup_df,
            on=["PaymentRemissionReason", "PaymentRemissionRequested"],
            how="left",
        )
        .join(silver_m4, on=["CaseNo"], how="left")
        .withColumn(
            "remissionType",
            when(col("remissionType") == lit("NO MAPPING REQUIRED"), None).otherwise(
                col("remissionType")
            ),
        )
        .withColumn(
        ...
        .withColumn(
            ...
            .when(
                col("legalAidAccountNumber")
                == lit("M1.LSCReference; ELSE IF NULL 'Unknown'"),
                when(
                    col("m1.LSCReference").isNotNull(),
                    when(
                        length(col("m1.LSCReference")) == 5,
                        concat(lit("0"), col("m1.LSCReference")),
                    )
                    .when(
                        length(col("m1.LSCReference")) == 4,
                        concat(lit("00"), col("m1.LSCReference")),
                    )
                    .when(
                        length(col("m1.LSCReference")) == 3,
                        concat(lit("000"), col("m1.LSCReference")),
                    )
                    .otherwise(col("m1.LSCReference")),
        ...
        .withColumn(
            ...
            when(
                col("asylumSupportReference") == lit("OMIT"),
                None,  # When set to OMIT, replace with NULL
            )
            .when(col("asylumSupportReference") == lit("NO MAPPING REQUIRED"), None)
            .when(
                col("asylumSupportReference")
                == lit(
                    "M1.ASFReferenceNo ELSE IF NULL 'Unknown'"
                ),  # If record matches the string
                when(
                    col("m1.ASFReferenceNo").isNotNull(), col("m1.ASFReferenceNo")
                ).otherwise(lit("Unknown")),  # Perform logic in the string
            )
        ...
        .withColumn(
            ...
            when(
                col("helpWithFeesReferenceNumber") == lit("OMIT"),
                None,  # As above
            )
            .when(
                col("helpWithFeesReferenceNumber") == lit("NO MAPPING REQUIRED"), None
            )
            .when(
                col("helpWithFeesReferenceNumber")
                == lit("M1.PaymentRemissionReasonNote; ELSE IF NULL 'Unknown'"),
                when(
                    col("m1.PaymentRemissionReasonNote").isNotNull(),
                    col("m1.PaymentRemissionReasonNote"),
                ).otherwise(lit("Unknown")),
            )
    ...
    df_audit = (
        silver_m1.alias("m1_audit")
        .filter(conditions_remissionTypes)
        .join(df.alias("content"), on=["CaseNo"], how="left")
        .select(
            col("CaseNo"),
            ## remissionType - ARIADM-784
            array(struct(*common_inputFields, lit("content.remissionType"))).alias(
                "remissionType_inputFields"
            ),
            array(struct(*common_inputValues, col("content.remissionType"))).alias(
                "remissionType_inputValues"
            ),
            col("content.remissionType"),
            lit("yes").alias("remissionType_Transformed"),
            ## remissionClaim - ARIADM-784
            array(struct(*common_inputFields, lit("content.remissionClaim"))).alias(
                "remissionClaim_inputFields"
            ),
            array(struct(*common_inputValues, col("content.remissionClaim"))).alias(
                "remissionClaim_inputValues"
            ),
            col("content.remissionClaim"),
            lit("yes").alias("remissionClaim_Transformed"),
            ## feeRemissionType - ARIADM-784
            array(struct(*common_inputFields, lit("content.feeRemissionType"))).alias(
                "feeRemissionType_inputFields"
            ),
            array(struct(*common_inputValues, col("content.feeRemissionType"))).alias(
                "feeRemissionType_inputValues"
            ),
            col("content.feeRemissionType"),
            lit("yes").alias("feeRemissionType_Transformed"),
            ## exceptionalCircumstances - ARIADM-786
            array(
                struct(*common_inputFields, lit("content.exceptionalCircumstances"))
            ).alias("exceptionalCircumstances_inputFields"),
            array(
                struct(*common_inputValues, col("content.exceptionalCircumstances"))
            ).alias("exceptionalCircumstances_inputValues"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("content.legalAidAccountNumber"),
                    lit("LSCReference"),
                )
            ).alias("legalAidAccountNumber_inputFields"),
            array(
                struct(
                    *common_inputValues,
            ...
            ).alias("legalAidAccountNumber_inputValues"),
            col("content.legalAidAccountNumber"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("content.asylumSupportReference"),
                    lit("ASFReferenceNo"),
                )
            ).alias("asylumSupportReference_inputFields"),
            array(
                struct(
                    *common_inputValues,
            ...
            ).alias("asylumSupportReference_inputValues"),
            col("content.asylumSupportReference"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("content.helpWithFeesReferenceNumber"),
                    lit("PaymentRemissionReasonNote"),
                )
            ).alias("helpWithFeesReferenceNumber_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("content.helpWithFeesReferenceNumber"),
                    col("PaymentRemissionReasonNote"),
                )
            ).alias("helpWithFeesReferenceNumber_inputValues"),
            col("content.helpWithFeesReferenceNumber"),
...
def sponsorDetails(silver_m1, silver_c):
    ...
    grouped = (
        grouped.withColumn(
            "hasSponsor",
            when(
                category_condition, when(name_condition, lit("Yes")).otherwise("No")
            ).otherwise(lit(None)),
        )
        .withColumn(
            "sponsorGivenNames",
        ...
        .withColumn(
            ...
            when(
                ...
                when(
                    name_condition,
                    when(col("Sponsor_Authorisation") == True, lit("Yes")).otherwise(
                        lit("No")
                    ),
                ).otherwise(lit(None)),
            ).otherwise(lit(None)),
        ...
        .withColumn(
            "sponsorEmailAdminJ",
            when((category_condition), cleanEmailUDF(col("Sponsor_Email"))),
        )
        .withColumn(
    ...
    df_audit = (
        ...
        .select(
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("audit.Sponsor_Name"),
                    lit("grp.CategoryIdList"),
                )
            ).alias("hasSponsor_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit.Sponsor_Name"),
                    col("grp.CategoryIdList"),
                )
            ).alias("hasSponsor_inputValues"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("audit.Sponsor_Forenames"),
                    lit("grp.CategoryIdList"),
                )
            ).alias("sponsorGivenName_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit.Sponsor_Forenames"),
                    col("grp.CategoryIdList"),
                )
            ).alias("sponsorGivenName_inputValues"),
            ...
            ).alias("sponsorFamilyName_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit.Sponsor_Name"),
            ...
            ).alias("sponsorFamilyName_inputValues"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("audit.Sponsor_Address1"),
                    lit("audit.Sponsor_Address2"),
                    lit("audit.Sponsor_Address3"),
                    lit("audit.Sponsor_Address4"),
                    lit("audit.Sponsor_Address5"),
                    lit("audit.Sponsor_Postcode"),
                    lit("grp.CategoryIdList"),
                )
            ).alias("sponsorAddress.inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit.Sponsor_Address1"),
                    col("audit.Sponsor_Address2"),
                    col("audit.Sponsor_Address3"),
                    col("audit.Sponsor_Address4"),
                    col("audit.Sponsor_Address5"),
                    col("audit.Sponsor_Postcode"),
                    col("grp.CategoryIdList"),
                )
            ).alias("sponsorAddress.inputValues"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("audit.Sponsor_Authorisation"),
                    lit("grp.CategoryIdList"),
                )
            ).alias("sponsorAuthorisation_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit.Sponsor_Authorisation"),
                    col("grp.CategoryIdList"),
                )
            ).alias("sponsorAuthorisation_inputValues"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("audit.Sponsor_Email"),
                    lit("grp.CategoryIdList"),
                )
            ).alias("sponsorEmailAdminJ_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit.Sponsor_Email"),
                    col("grp.CategoryIdList"),
                )
            ).alias("sponsorEmailAdminJ_inputValues"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("audit.Sponsor_Telephone"),
                    lit("grp.CategoryIdList"),
                )
            ).alias("sponsorMobileNumberAdminJ_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit.Sponsor_Telephone"),
                    col("grp.CategoryIdList"),
                )
            ).alias("sponsorMobileNumberAdminJ_inputValues"),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("audit.Sponsor_Name"),
                    lit("Sponsor_Forenames"),
                )
            ).alias("sponsorNameForDisplay_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit.Sponsor_Name"),
            ...
            ).alias("sponsorNameForDisplay_inputValues"),
            ...
            ).alias("sponsorAddressForDisplay_inputFields"),
            array(
                struct(
                    *common_inputValues,
                    col("audit.Sponsor_Address1"),
            ...
            ).alias("sponsorAddressForDisplay_inputValues"),
...
def general(
    ...
    conditions_general = (col("dv_representation").isin("LR", "AIP")) & (
        col("lu_appealType").isNotNull()
    )
    ...
    result_with_hearing_centre_df = (
        ...
        .withColumn(
            ...
            when(
                ...
                | (col("PaymentRemissionRequested") == lit("2")),
                "Yes",
            ).otherwise("No"),
        )
        .select(
            ...
            col("bhc.prevFileLocation").alias("dv_dhc_prevFileLocation"),
            col("bhc2.hearingCentre").alias("dv_dhc2_hearingCentre"),
            col("bhc2.applicationChangeDesignatedHearingCentre").alias(
                "dv_dhc2_applicationChangeDesignatedHearingCentre"
            ),
            col("bhc3.applicationChangeDesignatedHearingCentre").alias(
                "dv_dhc3_applicationChangeDesignatedHearingCentre"
            ),
            when(
                (
                    (col("bhc.Conditions").isNull())
                ...
                col("bhc.applicationChangeDesignatedHearingCentre"),
            )
            .when(
                (
                    col("h.der_prevFileLocation").isin(
                ...
                col("bhc3.applicationChangeDesignatedHearingCentre"),
            )
            .when(
                col("h.der_prevFileLocation").isin(
                    "Castle Park Storage",
            ...
            .otherwise(col("bhc2.applicationChangeDesignatedHearingCentre"))
            .alias("applicationChangeDesignatedHearingCentre"),
        )
    )

    df = (
        silver_m1.join(result_with_hearing_centre_df, ["CaseNo"], "left")
    ...
    df_audit = (
        ...
        .select(
            ...
            col("content.isServiceRequestTabVisibleConsideringRemissions"),
            lit("yes").alias(
                "isServiceRequestTabVisibleConsideringRemissions_Transformed"
            ),
            # Audit staffLocation
            array(
                struct(
...
def generalDefault(silver_m1):
    ...
    conditions_generalDefault = (col("dv_representation").isin("LR", "AIP")) & (
        col("lu_appealType").isNotNull()
    )

    df = (
        silver_m1.filter(conditions_generalDefault)
...
def documents(silver_m1):
    ...
    documents_audit = (
        silver_m1.alias("audit")
        .join(documents_content.alias("content"), on=["CaseNo"], how="left")
        .select(
            col("CaseNo"),
            # -----# uploadTheAppealFormDocs #-----#
            array(struct(*common_inputFields, lit("uploadTheAppealFormDocs"))).alias(
                "uploadTheAppealFormDocs_inputFields"
            ),
            array(struct(*common_inputValues, lit("null"))).alias(
                "uploadTheAppealFormDocs_inputValues"
            ),
            col("uploadTheAppealFormDocs"),
            lit("no").alias("uploadTheAppealFormDocs_Transformation"),
            # -----# caseNotes #-----#
            array(struct(*common_inputFields, lit("caseNotes"))).alias(
                "caseNotes_inputFields"
            ),
            array(struct(*common_inputValues, lit("null"))).alias(
                "caseNotes_inputValues"
            ),
            col("caseNotes"),
            lit("no").alias("caseNotes_Transformation"),
            # -----# tribunalDocuments #-----#
            array(struct(*common_inputFields, lit("tribunalDocuments"))).alias(
                "tribunalDocuments_inputFields"
            ),
            array(struct(*common_inputValues, lit("null"))).alias(
                "tribunalDocuments_inputValues"
            ),
            col("tribunalDocuments"),
            lit("no").alias("tribunalDocuments_Transformation"),
            # -----# legalRepresentativeDocuments #-----#
            array(
                struct(*common_inputFields, lit("legalRepresentativeDocuments"))
            ).alias("legalRepresentativeDocuments_inputFields"),
            array(struct(*common_inputValues, lit("null"))).alias(
                "legalRepresentativeDocuments_inputValues"
            ),
...
def caseState(silver_m1, desiredState):
    ...
    df_audit = (
        ...
        .select(
            ...
            array(struct(*common_inputFields, lit("ariaDesiredState"))).alias(
                "ariaDesiredState_inputFields"
            ),
            array(struct(*common_inputValues, lit("null"))).alias(
                "ariaDesiredState_inputValues"
            ),
            col("content.ariaDesiredState"),
            lit("no").alias("ariaDesiredState_Transformation"),
            # ariaMigrationTaskDueDays - ARIADM-797
            array(struct(*common_inputFields, lit("ariaMigrationTaskDueDays"))).alias(
                "ariaMigrationTaskDueDays_inputFields"
            ),
            array(struct(*common_inputValues, lit("null"))).alias(
                "ariaMigrationTaskDueDays_inputValues"
            ),
...
def detainedState(silver_m1):
    ...
    df_audit = (
        ...
        .select(
            col("CaseNo"),
            # ariaDesiredState - ARIADM-797
            array(struct(*common_inputFields, lit("appellantInDetention"))).alias(
                "appellantInDetention_inputFields"
            ),
            array(struct(*common_inputValues, lit("No"))).alias(
                "appellantInDetention_inputValues"
            ),
...
def paymentPendingOnly(silver_m1):
    df_final = silver_m1.withColumn(
        "completeCaseReviewDate", date_format(col("DateLodged"), "yyyy-MM-dd")
    ).select("CaseNo", "completeCaseReviewDate")
    ...
    df_audit = (
        ...
        .select(
            col("CaseNo"),
            array(struct(*common_inputFields)).alias(
                "completeCaseReviewDate_inputFields"
            ),
            array(struct(*common_inputValues)).alias(
                "completeCaseReviewDate_inputValues"
            ),
