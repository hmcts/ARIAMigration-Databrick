...
# Module-level clients to reuse across function invocations
...
# Semaphore for limiting concurrency inside a batch
...
@app.event_hub_message_trigger(
    ...
    prefetch_count=2000,    # Prefetch to improve batch fetch
    data_type='binary'
)
async def eventhub_trigger_bails(azeventhub: List[func.EventHubEvent]):
    ...
    try:
        ...
        container_secret = (await kv_client.get_secret(f"CURATED-AZUREFUNCTION-{env}-SAS-TOKEN")).value #AM 030625
        ...
        dl_producer_client = None
        ack_producer_client = None
        ...
        try:
            ...
            try:
                dl_producer_client = EventHubProducerClient.from_connection_string(ev_dl_key)
                ack_producer_client = EventHubProducerClient.from_connection_string(ev_ack_key)

                async with dl_producer_client, ack_producer_client:
                    logging.info('Processing messages')

                    # Limit concurrency using semaphore to prevent event-loop overload
                    async def sem_task(event):
                        async with sem:
                            return await process_messages(event, container_service_client, sub_dir, dl_producer_client, ack_producer_client)

                    tasks = [sem_task(event) for event in azeventhub]
                    await asyncio.gather(*tasks)

                    logging.info('Finished processing messages')

            finally:
                if dl_producer_client is not None:
                    await dl_producer_client.close()
                if ack_producer_client is not None:
                    await ack_producer_client.close()
        finally:
            if container_service_client is not None:
                await container_service_client.close()

    finally:
        # Explicitly close SecretClient and credential to avoid session leaks
