...
async def eventhub_trigger_active(azeventhub: List[func.EventHubEvent]):
    ...
    idempotency_account_url = (
        f"https://ingest{LZ_KEY}xcutting{ENV}.blob.core.windows.net"
    )
    idempotency_container_name = "af-idempotency"
    idempotency_blob_service = BlobServiceClient(
        account_url=idempotency_account_url, credential=credential
    )
    idempotency_container = idempotency_blob_service.get_container_client(
        idempotency_container_name
    )
    ...
    async with res_eh_producer:
        ...
        try:
            for event in azeventhub:
                try:
                    logging.info(
                        f'Event received with partition key: '
                        f'{event.partition_key}'
                    )
                    ...
                    idempotency_blob_path = (
                        f"active/{state}/processed/{caseNo}.flag"
                    )
                    ...
                    if await idempotency_blob.exists():
                        logging.warning(
                            f"[IDEMPOTENCY] Skipping duplicate message "
                            f"for {state}/{caseNo}"
                        )
                        continue
                    ...
                    result = await asyncio.to_thread(
                        process_case, ENV, caseNo,
                        data, run_id, state, PR_NUMBER
                    )
                    ...
                    if result.get("Status") == "Success":
                        try:
                            await idempotency_blob.upload_blob(
                                b"", overwrite=True
                            )
                            logging.info(
                                f"[IDEMPOTENCY] Marked processed: {caseNo}"
                            )
                        except Exception as upload_error:
                            logging.error(
                                f"[IDEMPOTENCY] Failed to mark processed "
                                f"for {caseNo}: {upload_error}"
                            )
                    ...
                    try:
                        ...
                        logging.info(
                            'Sent a batch of events to Results Event Hub'
                        )
                        event_data_batch = await res_eh_producer.create_batch()
                        event_data_batch.add(EventData(result_json))
                    ...
                    logging.error(
                        f'Error processing event for caseNo {caseNo}: {e}'
                    )
            ...
            if len(event_data_batch) > 0:
                await res_eh_producer.send_batch(event_data_batch)
                logging.info(
                    'Sent the final batch of events to Results Event Hub'
                )
