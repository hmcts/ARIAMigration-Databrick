from pyspark.sql.functions import array, col, concat, lit, row_number, struct
...
def ftpa(silver_m1, silver_m2, silver_m3, silver_c):
    ...
    ftpa_audit = (
        ...
        .select(
            "audit.*",
            array(struct(lit("judgeAllocationExists"))).alias(
                "judgeAllocationExists_inputFields"
            ),
            ...
            array(
                struct(lit("Adj_Title"), lit("Adj_Forenames"), lit("Adj_Surname"))
            ).alias("allocatedJudge_inputFields"),
            array(
                struct(col("Adj_Title"), col("Adj_Forenames"), col("Adj_Surname"))
            ).alias("allocatedJudge_inputValues"),
            col("allocatedJudge").alias("allocatedJudge_value"),
            lit("Yes").alias("allocatedJudge_Transformation"),
            array(
                struct(lit("Adj_Title"), lit("Adj_Forenames"), lit("Adj_Surname"))
            ).alias("allocatedJudgeEdit_inputFields"),
            array(
                struct(col("Adj_Title"), col("Adj_Forenames"), col("Adj_Surname"))
            ).alias("allocatedJudgeEdit_inputValues"),
