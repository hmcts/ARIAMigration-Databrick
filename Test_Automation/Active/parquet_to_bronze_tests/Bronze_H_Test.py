...
#############################
#TEST H bronze_history
#############################

def test_h_bronze_table(config):

    TEST_NAME = "H_BRONZE"
    BRONZE_TABLE_NAME = "ariadm_active_appeals.bronze_history"

    #Tables and Field includes and Renames
    H_TABLES_CONFIG = [
        {
            "name": "History",
            "include" : ["HistoryId", "CaseNo","HistType", "Comment"],
            "renames": {        
            }
        }
    ]
    ...
    pqdfs = config["pqhelper"].load_sql_tables_into_df(H_TABLES_CONFIG,config["PARQUET_BASE_PATH"],config["dbutils"])

    #Create DF of SQL table
    sql_joined_df = pqdfs["History"]        
