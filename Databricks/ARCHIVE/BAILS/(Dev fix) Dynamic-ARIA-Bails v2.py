...
m1 = spark.read.table("hive_metastore.aria_bails.silver_bail_m1_case_details").filter(col("CaseNo").isin(test_case_no)).dropDuplicates(["CaseNo"]).alias("m1")
...
linked_cases = spark.read.table("hive_metastore.aria_bails.linked_cases_cost_award").filter(col("CaseNo").isin(test_case_no))
...
# History 
m5_filtered = m5.filter(col("CaseNo") == case_number)
...
# COMMAND ----------
    ...
    # Appeal Category
m8_filtered = m8.filter(col("CaseNo") == case_number)
