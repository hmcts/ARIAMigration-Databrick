...
def run_all_tests(json_data, M1_bronze, M1_silver, M2_bronze, M3_bronze, C, bhc, fields_to_exclude):
    ...
    test_df, test_data_setup =  are_b_tests.test_default_mapping_init(json_data)
    if test_data_setup != True:
         all_test_results.append(test_data_setup)

    if test_df != None:
        all_test_results.extend(are_b_tests.test_AREB_defaultValues(test_df, fields_to_exclude))
