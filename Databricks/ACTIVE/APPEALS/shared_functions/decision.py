from pyspark.sql.functions import array, col, concat, lit, regexp_replace, struct
...
def substantiveDecision(silver_m1):
    substantiveDecision_df = (
        silver_m1.select(col("CaseNo"))
        .withColumn("scheduleOfIssuesAgreement", lit("No"))
        .withColumn(
            "scheduleOfIssuesDisagreementDescription",
            lit(
                "This is a migrated ARIA case. Please see the documents for information on the schedule of issues."
            ),
        )
        ...
        .withColumn(
            ...
            lit(
                "This is a migrated ARIA case. Please see the documents for information on the immigration history."
            ),
        )
    )
    ...
    substantiveDecision_audit = (
        ...
        .select(
            ...
            array(struct(*common_inputFields, lit("scheduleOfIssuesAgreement"))).alias(
                "scheduleOfIssuesAgreement_inputFields"
            ),
            array(struct(*common_inputValues, lit("null"))).alias(
                "scheduleOfIssuesAgreement_inputValues"
            ),
            col("scheduleOfIssuesAgreement"),
            lit("Yes").alias("scheduleOfIssuesAgreement_Transformation"),
            array(
                struct(
                    *common_inputFields, lit("scheduleOfIssuesDisagreementDescription")
                )
            ).alias("scheduleOfIssuesDisagreementDescription_inputFields"),
            array(struct(*common_inputValues, lit("null"))).alias(
                "scheduleOfIssuesDisagreementDescription_inputValues"
            ),
            col("scheduleOfIssuesDisagreementDescription"),
            lit("Yes").alias("scheduleOfIssuesDisagreementDescription_Transformation"),
            array(
                struct(*common_inputFields, lit("immigrationHistoryAgreement"))
            ).alias("immigrationHistoryAgreement_inputFields"),
            array(struct(*common_inputValues, lit("null"))).alias(
                "immigrationHistoryAgreement_inputValues"
            ),
            ...
            array(
                struct(
                    *common_inputFields,
                    lit("immigrationHistoryDisagreementDescription"),
                )
            ).alias("immigrationHistoryDisagreementDescription_inputFields"),
            array(struct(*common_inputValues, lit("null"))).alias(
                "immigrationHistoryDisagreementDescription_inputValues"
            ),
            col("immigrationHistoryDisagreementDescription"),
            lit("Yes").alias(
                "immigrationHistoryDisagreementDescription_Transformation"
            ),
        )
    )
...
def generalDefault(silver_m1):
    ...
    general_df = (
        general_df.withColumn("hmcts", lit("[userImage:hmcts.png]"))
...
def general(
    ...
    general_df, general_audit = L.general(
        silver_m1,
        silver_m2,
        silver_m3,
        silver_h,
    ...
    general_df = general_df.join(bundleFileNamePrefix_df, on="CaseNo", how="left").drop(
        "Appellant_Name"
    )
    ...
    general_audit = (
        ...
        .select(
            ...
            array(struct(lit("CaseNo"), lit("Appellant_Name"))).alias(
                "bundleFileNamePrefix_inputFields"
            ),
            # bundleFileNamePrefix_inputValues
            array(struct(col("audit.CaseNo"), col("bfp.Appellant_Name"))).alias(
                "bundleFileNamePrefix_inputValues"
            ),
            # Transformed fields
