...
def run_all_tests(json_data, M1_bronze, M1_silver, M2_bronze, M3_bronze, C, bhc, bac, bll, b, fields_to_exclude, child_fields_to_exclude):
    ...
    active_b = b if b is not None else M1_bronze
    active_bac = bac if bac is not None else C
    test_df, test_data_setup =  list_tests.test_default_mapping_init(json_data, M1_silver, active_bac, active_b)
    ...
    if test_df is not None:
      if not any(field in fields_to_exclude for field in language_fields):
