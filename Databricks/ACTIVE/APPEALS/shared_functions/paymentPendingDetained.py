...
def detained(silver_m1, silver_m2, bronze_detention_centres):
    ...
    joined_m1_m2 = silver_m1.alias("m1").join(
        silver_m2.alias("m2"), on="CaseNo", how="left"
    )

    detained_df = (
        joined_m1_m2.alias("m2")
        .join(bronze_detention_centres.alias("det"), on="DetentionCentreId", how="left")
        .withColumn(
            "appellantInDetention",
            when(col("m2.Detained").isin([1, 2, 4]), "Yes").otherwise(lit("No")),
        )
        .withColumn(
            "detentionFacility",
        ...
        .withColumn(
            "prisonNOMSNumber",
            when(
                (col("m2.Detained") == 1) & (col("m2.PrisonRef").isNotNull()),
        ...
        .withColumn(
            ...
            when(
                ...
                struct(
                    coalesce(
                        col("det.DetentionCentre"), col("m2.Appellant_Address1")
                    ).alias("other")
                ),
            ).otherwise(None),
        )
        .withColumn(
            "ircName",
            when((col("m2.Detained") == 2), col("det.ircName")).otherwise(None),
        )
        .withColumn(
            "releaseDateProvided",
            when((col("m2.Detained").isin(1, 4)), lit("Yes")).otherwise(None),
        )
        .withColumn(
            "hasPendingBailApplications",
            when((col("m2.Detained") == 2), lit("NotSure")).otherwise(None),
        )
        .withColumn(
            "removalOrderOptions",
            when((col("m2.RemovalDate").isNotNull()), lit("Yes")).otherwise("No"),
        )
        .withColumn(
            "removalOrderDate",
            when(
                (col("m2.RemovalDate").isNotNull()),
                date_format(col("m2.RemovalDate"), "yyyy-MM-dd'T'HH:mm:ss.SSS"),
            ).otherwise(None),
        )
        .withColumn(
            "detentionBuilding",
            when(
                (col("m2.Detained").isin(1, 2)), col("det.detentionBuilding")
            ).otherwise(None),
        )
        .withColumn(
            "detentionAddressLines",
            when(
                (col("m2.Detained").isin(1, 2)), col("det.detentionAddressLines")
            ).otherwise(None),
        )
        .withColumn(
            "detentionPostcode",
            when(
                (col("m2.Detained").isin(1, 2)), col("det.detentionPostcode")
            ).otherwise(None),
        )
        .select(
            col("CaseNo"),
    ...
    detained_audit = (
        ...
        .join(
            bronze_detention_centres.alias("det"),
            col("m1_m2.DetentionCentreId") == col("det.DetentionCentreId"),
            "left",
        )
        ...
        .select(
            ...
            array(struct(col("m1_m2.Detained"))).alias(
            ...
            array(struct(lit("Detained"), lit("PrisonRef"))).alias(
                "prisonNOMSNumber_inputFields"
            ),
            array(struct(col("m1_m2.Detained"), col("m1_m2.PrisonRef"))).alias(
                "prisonNOMSNumber_inputValues"
            ),
            col("content.prisonNOMSNumber"),
            lit("Yes").alias("prisonNOMSNumber_Transformation"),
            array(
                struct(
                    lit("Detained"), lit("DetentionCentre"), lit("Appellant_Address1")
                )
            ).alias("otherDetentionFacilityName_inputFields"),
            array(
                struct(
                    col("m1_m2.Detained"),
            ...
            array(struct(col("m1_m2.Detained"), col("det.ircName"))).alias(
                "ircName_inputValues"
            ),
            col("content.ircName"),
            ...
            array(struct(col("m1_m2.Detained"))).alias(
                "releaseDateProvided_inputValues"
            ),
            col("content.releaseDateProvided"),
            lit("Yes").alias("releaseDateProvided_Transformation"),
            array(struct(lit("Detained"))).alias(
                "hasPendingBailApplications_inputFields"
            ),
            array(struct(col("m1_m2.Detained"))).alias(
                "hasPendingBailApplications_inputValues"
            ),
            ...
            array(struct(col("m1_m2.RemovalDate"))).alias(
                "removalOrderOptions_inputValues"
            ),
            ...
            array(struct(col("m1_m2.RemovalDate"))).alias(
                "removalOrderDate_inputValues"
            ),
            col("content.removalOrderDate"),
            lit("Yes").alias("removalOrderDate_Transformation"),
            array(struct(lit("Detained"), lit("detentionBuilding"))).alias(
                "detentionBuilding_inputFields"
            ),
            array(struct(col("m1_m2.Detained"), col("det.detentionBuilding"))).alias(
                "detentionBuilding_inputValues"
            ),
            col("content.detentionBuilding"),
            lit("Yes").alias("detentionBuilding_Transformation"),
            array(struct(lit("Detained"), lit("detentionAddressLines"))).alias(
                "detentionAddressLines_inputFields"
            ),
            array(
                struct(col("m1_m2.Detained"), col("det.detentionAddressLines"))
            ).alias("detentionAddressLines_inputValues"),
            col("content.detentionAddressLines"),
            lit("Yes").alias("detentionAddressLines_Transformation"),
            array(struct(lit("Detained"), lit("detentionPostcode"))).alias(
                "detentionPostcode_inputFields"
            ),
            array(struct(col("m1_m2.Detained"), col("det.detentionPostcode"))).alias(
                "detentionPostcode_inputValues"
            ),
...
def caseData(
    ...
    caseData_df, caseData_audit = PP.caseData(
        silver_m1,
        silver_m2,
        silver_m3,
        silver_h,
    ...
    caseData_df = (
        joined_m1_m2.alias("m2")
        .join(caseData_df.alias("content"), on="CaseNo", how="left")
        .join(bronze_detention_centres.alias("det"), on="DetentionCentreId", how="left")
        .join(
            bronze_hearing_centres.alias("bhc"),
            on=col("m2.CentreId") == col("bhc.CentreId"),
            how="left",
        )
        .withColumn(
            "hearingCentre1",
            when(col("m2.Detained").isin(1, 2), col("det.hearingCentre")).otherwise(
                col("content.hearingCentre")
            ),
        )
        .withColumn(
            "staffLocation1",
            when(col("m2.Detained").isin(1, 2), col("det.staffLocation")).otherwise(
                col("content.staffLocation")
            ),
        )
        .withColumn(
            "caseManagementLocation1",
            when(
                col("m2.Detained").isin(1, 2), col("det.caseManagementLocation")
            ).otherwise(col("content.caseManagementLocation")),
        )
        .withColumn(
            "hearingCentreDynamicList1",
            when(
                col("m2.Detained").isin(1, 2),
                struct(
                    struct(
                    ...
                    array(
                        struct(
                            ...
                            lit("Newport Tribunal Centre - Columbus House").alias(
                                "label"
                            ),
                        ),
                        struct(
        ...
        .withColumn(
            "caseManagementLocationRefData1",
            when(
                col("m2.Detained").isin(1, 2),
                struct(
                    lit("1").alias("region"),
                    struct(
                        ...
                        array(
                            ...
                            struct(
                                lit("386417").alias("code"),
                                lit("Hatton Cross Tribunal Hearing Centre").alias(
                                    "label"
                                ),
                            ),
                            struct(
                            ...
                            struct(
                                lit("765324").alias("code"),
                                lit("Taylor House Tribunal Hearing Centre").alias(
                                    "label"
                                ),
                            ),
                        ).alias("list_items"),
        ...
        .withColumn(
            "selectedHearingCentreRefData1",
            when(
                col("m2.Detained").isin(1, 2), col("det.selectedHearingCentreRefData")
            ).otherwise(col("content.selectedHearingCentreRefData")),
        )
        .drop(
            "hearingCentre",
            "staffLocation",
        ...
        .select(
            ...
            col("caseManagementLocationRefData1").alias(
                "caseManagementLocationRefData"
            ),
...
def general(
    ...
    general_df, general_audit = PP.general(
        silver_m1,
        silver_m2,
        silver_m3,
        silver_h,
    ...
    general_df = (
        ...
        .withColumn(
            "applicationChangeDesignatedHearingCentre1",
            when(
                col("m2.Detained").isin(1, 2),
                col("det.applicationChangeDesignatedHearingCentre"),
        ...
        .withColumn(
            ...
            struct(
                ...
                date_format(date_add(col("m2.DateLodged"), 36524), "yyyy-MM-dd").alias(
                    "SystemTTL"
                ),
            ),
        )
        ...
        .select(
            "content.*",
            col("applicationChangeDesignatedHearingCentre1").alias(
...
def appellantDetails(
    ...
    appellantDetails_df, appellantDetails_audit = PP.appellantDetails(
        silver_m1,
        silver_m2,
        silver_c,
        bronze_countryFromAddress,
    ...
    appellantDetails_df = appellantDetails_df.drop(
        "appellantInUk",
        "appealOutOfCountry",
    ...
    silver_m1_derived = silver_m1.select(
        col("CaseNo"),
        col("dv_representation"),
        col("lu_appealType"),
        col("HORef"),
        col("DateOfApplicationDecision"),
    )
    ...
    appellantDetails_df = (
        ...
        .withColumn(
            "appellantHasFixedAddressAdminJ",
            when(conditions & (col("appellantInUk") == "No"), lit("Yes")).otherwise(
                lit(None)
            ),
        )
        # -----------------------------
        ...
        .withColumn(
            ...
            when(
                conditions & (col("appellantInUk") == "No"),
                when(
                    col("countryGovUkOocAdminJ").isin(["NO MAPPING REQUIRED"]),
                    lit(None),
...
def homeOfficeDetails(silver_m1, silver_m2, silver_c, bronze_HORef_cleansing):
    ...
    homeOfficeDetails_df, homeOfficeDetails_audit = PP.homeOfficeDetails(
    ...
    is_detained_or_in_uk = col("m2.Detained").isin(1, 2, 4) | col(
        "m2.dv_appellantIsInUk"
    )
    ...
    decision_letter_received_date_expr = when(
        ...
        & (
            ~coalesce(
                col("bc.lu_HORef"), col("m1.HORef"), col("m2.FCONumber"), lit("")
            ).like("%GWF%")
        )
    ...
    homeOfficeDetails_df = (
        ...
        )  # if both HO and GWF Ref are NULL, set default for HO Ref Number.
...
def sponsorDetails(silver_m1, silver_m2, silver_c):
    ...
    sponsorDetails_df = silver_m2_derived_grouped.alias("m2").join(
        m1.alias("m1"), on="CaseNo", how="left"
    )
    ...
    sponsorDetails_df = (
        sponsorDetails_df.withColumn(
            "hasSponsor", when(name_condition, lit("Yes")).otherwise("No")
        )
        .withColumn(
            "sponsorGivenNames",
        ...
        .withColumn(
            "sponsorFamilyName",
            when((name_condition), col("Sponsor_Name")).otherwise(lit(None)),
        )
        .withColumn(
        ...
        .withColumn(
            "sponsorEmailAdminJ",
            when((name_condition), cleanEmailUDF(col("Sponsor_Email"))),
        )
        .withColumn(
            "sponsorMobileNumberAdminJ",
            when(
                (name_condition), filterMobilePhoneNumberUDF(col("Sponsor_Telephone"))
            ),
        )
        .withColumn(
            ...
            when(
                name_condition,
                concat_ws(" ", col("Sponsor_Forenames"), col("Sponsor_Name")),
            ).otherwise(lit(None)),
        )
        .withColumn(
    ...
    df_audit = (
        ...
        .join(sponsorDetails_df.alias("grp"), ["CaseNo"], "left")
        .select(
            col("CaseNo"),
            # audit hasSponsor
            array(
...
def partyID(silver_m1, silver_m2, silver_m3, silver_c):
    ...
    party_df_new = (
        silver_m2_derived_grouped.alias("m2")
    ...
    df = (
        party_df_new.withColumn(
            "sponsorPartyId",
            when(col("m1.Sponsor_Name").isNotNull(), expr("uuid()")).otherwise(None),
        )
