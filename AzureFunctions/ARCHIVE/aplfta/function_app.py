...
import aiohttp  # Added to download from HTTPS path
...
async def process_messages(event,container_service_client,subdirectory,dl_producer_client,ack_producer_client):
        ...
        try:
            ...
            # FIX START: handle HTTPS URL instead of raw content
            if message.startswith("https://"):
                logging.info(f"Downloading blob content from {message}")
                async with aiohttp.ClientSession() as session:
                    async with session.get(message) as response:
                        if response.status != 200:
                            raise Exception(f"Failed to download source blob: {response.status}")
                        data = await response.read()
                        await upload_blob_with_retry(blob_client, data, capture_response)
                logging.info(f"Uploaded file from URL: {key}")
            else:
                # fallback: treat as old content-style message
                await upload_blob_with_retry(blob_client, message, capture_response)
                logging.info(f"Uploaded file from inline content: {key}")
            # FIX END
