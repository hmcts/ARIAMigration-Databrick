from pyspark.sql.functions import (
    array, array_compact, array_contains, array_distinct, array_size, col, collect_list, expr, lit, row_number, struct, when
)
    ...
    interpreter_languages_lookup = (
        silver_m1.alias("m1")
            ...
            .withColumn("lu_appellantInterpreterSpokenLanguage",
                when((array_contains(col("lu_appellantInterpreterLanguageCategory"), spokenLanguageCategory)),
                    struct(
                        when((col("lu_appellantInterpreterSpokenLanguageRefData").isNotNull()) & (array_size(col("lu_appellantInterpreterSpokenLanguageRefData.value")) > 0),
                            col("lu_appellantInterpreterSpokenLanguageRefData")
                        ).otherwise(lit(None)).alias("languageRefData"),
                        when((col("lu_appellantInterpreterSpokenLanguageRefData").isNotNull()) & (array_size(col("lu_appellantInterpreterSpokenLanguageRefData.value")) > 0),
                            spoken_languages_list_literal
                        ).otherwise(lit(None)).alias("list_items"),
                        col("lu_spokenManualEntry").alias("languageManualEntry"),
                        col("lu_spokenManualEntryDescription").alias("languageManualEntryDescription")
                    )
                )
            )
            .withColumn("lu_appellantInterpreterSignLanguage",
                when((array_contains(col("lu_appellantInterpreterLanguageCategory"), signLanguageCategory)),
                    struct(
                        when((col("lu_appellantInterpreterSignLanguageRefData").isNotNull()) & (array_size(col("lu_appellantInterpreterSignLanguageRefData.value")) > 0),
                        ...
                        col("lu_signManualEntryDescription").alias("languageManualEntryDescription")
                    )
                )
            )
            .select(
