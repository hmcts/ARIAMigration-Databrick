...
# Singletons — created once per process, reused across invocations.
# DefaultAzureCredential caches tokens and auto-refreshes before expiry.
...
async def eventhub_trigger_bails(azeventhub: List[func.EventHubEvent]):
    ...
    # Fetch all KV secrets in parallel instead of sequentially
    ...
    try:
        # BlobServiceClient is from azure.storage.blob.aio — must be async with, not with
...
async def process_messages(event, container_service_client, subdirectory, dl_producer_client, ack_producer_client, source_container_secret, idempotency_container):
    ...
    try:
        ...
        # Idempotency check — uses shared container client passed in from the batch trigger
        try:
